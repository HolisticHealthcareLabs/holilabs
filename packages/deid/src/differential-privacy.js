...
exports.dpSum = dpSum;
exports.dpAverage = dpAverage;
exports.composeEpsilon = composeEpsilon;
/**
 ...
 */
function dpCount(value, epsilon, sensitivity = 1) {
    if (epsilon <= 0) {
 ...
 */
function dpHistogram(counts, epsilon, sensitivity = 1) {
    if (epsilon <= 0) {
        throw new Error('Epsilon must be positive');
    }
    const result = {};
 ...
 * @returns Noisy sum
 */
function dpSum(value, epsilon, sensitivity) {
    if (epsilon <= 0) {
        throw new Error('Epsilon must be positive');
    }
    const scale = sensitivity / epsilon;
 ...
 */
function dpAverage(sum, count, epsilon, sensitivity) {
    if (epsilon <= 0) {
        throw new Error('Epsilon must be positive');
    }
 ...
 */
function composeEpsilon(epsilons) {
