...
export function generalize(
  ...
  function generalizeValue(value: any, key?: string): any {
 ...
 * Map age to age band
 */
function getAgeBand(age: number, bands: string[]): string {
  for (const band of bands) {
    const [min, max] = band.split('-').map((s) => {
      if (s.endsWith('+')) return [parseInt(s), Infinity];
      return parseInt(s);
    });

    if (Array.isArray(min)) {
      if (age >= min[0]) return band;
 ...
 */
function generalizeDate(dateStr: string, mode: 'YEAR' | 'YEAR_OR_QUARTER'): string {
  try {
 ...
 */
function generalizeGeo(geo: string, mode: 'ZIP3_OR_STATE' | 'STATE_ONLY'): string {
  if (mode === 'STATE_ONLY') {
 ...
 */
function isAgeField(fieldName: string): boolean {
