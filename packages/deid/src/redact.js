...
exports.redactStructured = redactStructured;
 ...
 */
function redactStructured(data, policy) {
    const counts = {};
 ...
 */
function redactText(text, locale, policy) {
    let redacted = text;
    for (const identifier of policy.redaction.identifiers) {
        const patterns = IDENTIFIER_PATTERNS[identifier];
        if (patterns) {
    ...
    if (policy.text_nlp.fallback === 'REDACT') {
        ...
        if (policy.text_nlp.min_confidence > 0.9) {
            redacted = redacted.replace(/\b[A-ZÁÉÍÓÚÑÀÂÃÊÔÕÇ][a-záéíóúñàâãéêíóôõúç]{2,}\s+[A-ZÁÉÍÓÚÑÀÂÃÊÔÕÇ][a-záéíóúñàâãéêíóôõúç]{2,}\b/g, '[NAME_REDACTED]');
        }
    }
    return redacted;
}
 ...
 */
function shouldSuppressField(fieldName, policy) {
