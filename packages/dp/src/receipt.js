 ...
 */
function generateReceipt(datasetSha256, epsilon, delta, policyVersion, orgId, subjectId) {
...
async function generateReceiptPDF(receipt) {
    return new Promise((resolve, reject) => {
        ...
        doc.text('This receipt certifies that the exported dataset has been protected with differential privacy. ' +
            'The epsilon (ε) and delta (δ) parameters quantify the privacy guarantee provided. ' +
            'Lower values indicate stronger privacy protection.', { align: 'justify' });
        doc.moveDown();
        doc.text('This receipt is cryptographically signed and can be independently verified. ' +
            'Store this receipt for compliance and audit purposes.', { align: 'justify' });
        // Footer
        doc.moveDown(2);
