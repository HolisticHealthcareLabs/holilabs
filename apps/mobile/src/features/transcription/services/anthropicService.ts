...
import { Transcription, TranscriptionSegment } from '@/shared/types';
...
export class AnthropicService {
   ...
   */
  async transcribeAudio(audioUri: string, patientContext?: string): Promise<Transcription> {
    try {
   ...
   */
  async generateSOAPNote(transcription: Transcription): Promise<any> {
    try {
      ...
      return {
        ...
        diagnoses: [
          { code: 'R51', description: 'Headache', type: 'primary' as const },
        ],
      };
    } catch (error) {
