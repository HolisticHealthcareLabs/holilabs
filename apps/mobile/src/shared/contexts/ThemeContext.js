...
const ThemeProvider = ({ children }) => {
    ...
    const [themeMode, setThemeModeState] = (0, react_1.useState)('auto');
    // Load saved theme preference on mount
    (0, react_1.useEffect)(() => {
        const loadTheme = async () => {
            const saved = storage_1.storage.getString('theme_mode');
            if (saved) {
    ...
    const setThemeMode = async (mode) => {
        ...
        storage_1.storage.set('theme_mode', mode);
    };
    // Determine if we should use dark mode
    const isDark = themeMode === 'dark' ||
        (themeMode === 'auto' && systemColorScheme === 'dark');
    const theme = isDark ? theme_1.darkTheme : theme_1.lightTheme;
    return (<ThemeContext.Provider value={{ theme, themeMode, setThemeMode, isDark }}>
      {children}
