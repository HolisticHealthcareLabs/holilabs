...
async function seedCredentials() {
    ...
    try {
        ...
        for (const credential of sampleCredentials) {
            ...
            created++;
            console.log(`✓ Created: ${credential.credentialType} - ${credential.credentialNumber} (${credential.verificationStatus})`);
        }
        console.log(`\n✅ Successfully seeded ${created} credentials!`);
        console.log('\nCredential Summary:');
        console.log(`- VERIFIED: ${sampleCredentials.filter((c) => c.verificationStatus === 'VERIFIED').length}`);
        console.log(`- PENDING: ${sampleCredentials.filter((c) => c.verificationStatus === 'PENDING').length}`);
        console.log(`- MANUAL_REVIEW: ${sampleCredentials.filter((c) => c.verificationStatus === 'MANUAL_REVIEW').length}`);
        console.log(`- Auto-verified: ${sampleCredentials.filter((c) => c.autoVerified).length}`);
        console.log(`- Manually verified: ${sampleCredentials.filter((c) => c.manualVerified).length}`);
