...
function generateMRN() {
    return `PT-${faker_1.faker.string.alphanumeric(4)}-${faker_1.faker.string.alphanumeric(4)}-${faker_1.faker.string.alphanumeric(4)}`.toUpperCase();
}
function generateDataHash(patientData) {
    return crypto_1.default.createHash('sha256').update(JSON.stringify(patientData)).digest('hex');
}
...
async function generatePatient(locale, clinicianId, index) {
    ...
    if (ageCategory < 0.2) {
        age = faker_1.faker.number.int({ min: 1, max: 17 }); // Pediatric
    }
    else if (ageCategory < 0.8) {
        age = faker_1.faker.number.int({ min: 18, max: 64 }); // Adult
    }
    else {
        age = faker_1.faker.number.int({ min: 65, max: 88 }); // Elderly
    }
    ...
    const tokenId = `PT-${faker_1.faker.string.alphanumeric(4)}-${faker_1.faker.string.alphanumeric(4)}-${faker_1.faker.string.alphanumeric(4)}`.toLowerCase();
    // Generate medical history based on age
    const conditions = [];
    const medications = [];
    if (age >= 40) {
        // Adults more likely to have chronic conditions
        const conditionCount = faker_1.faker.number.int({ min: 0, max: 3 });
    ...
    const allergies = [];
    if (Math.random() < 0.4) {
        const allergyCount = faker_1.faker.number.int({ min: 1, max: 2 });
    ...
    for (const med of medications) {
        await prisma.medication.create({
            data: {
                ...
                startDate: faker_1.faker.date.past({ years: 2 }),
                isActive: true,
            },
        });
    }
    ...
    if (Math.random() < 0.6) {
        const appointmentDate = faker_1.faker.date.soon({ days: 14 });
...
//# sourceMappingURL=seed-patients.js.map