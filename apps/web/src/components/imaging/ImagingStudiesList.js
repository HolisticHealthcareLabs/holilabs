...
exports.default = ImagingStudiesList;
...
function ImagingStudiesList({ patientId }) {
    const [imagingStudies, setImagingStudies] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [error, setError] = (0, react_1.useState)(null);
    // Filters
    const [modalityFilter, setModalityFilter] = (0, react_1.useState)('ALL');
    const [statusFilter, setStatusFilter] = (0, react_1.useState)('ALL');
    const [abnormalFilter, setAbnormalFilter] = (0, react_1.useState)('ALL');
    // Selected study for detail view
    const [selectedStudy, setSelectedStudy] = (0, react_1.useState)(null);
    // Image viewer
    const [viewingImage, setViewingImage] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        fetchImagingStudies();
    ...
    const fetchImagingStudies = async () => {
        try {
            ...
            if (abnormalFilter !== 'ALL')
                params.append('isAbnormal', abnormalFilter);
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <select value={modalityFilter} onChange={(e) => setModalityFilter(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
          ...
          <div>
            ...
            <select value={abnormalFilter} onChange={(e) => setAbnormalFilter(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              <option value="ALL">Todos</option>
      ...
      {/* Studies Grid */}
      {imagingStudies.length === 0 ? (<div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
          <p className="text-gray-600">No se encontraron estudios de imágenes</p>
        </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {imagingStudies.map((study) => (<div key={study.id} onClick={() => handleStudyClick(study)} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow cursor-pointer">
              {/* Thumbnail */}
              <div className="relative h-48 bg-gray-100 flex items-center justify-center">
                {study.thumbnailUrl ? (<img src={study.thumbnailUrl} alt={study.description} className="w-full h-full object-cover"/>) : (<div className="text-6xl">{modalityIcons[study.modality] || '📷'}</div>)}
                {study.imageCount > 0 && (<div className="absolute bottom-2 right-2 bg-black bg-opacity-75 text-white px-2 py-1 rounded text-xs font-medium">
              ...
              <div className="p-4 space-y-3">
                <div>
                  <div className="flex items-start justify-between mb-2">
                    ...
                    {study.isAbnormal && (<span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800 border border-orange-300 ml-2">
                        Anormal
                      </span>)}
                  </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-600">
                    {(0, date_fns_1.format)(new Date(study.studyDate), 'd MMM yyyy', { locale: locale_1.es })}
                  </span>
                </div>

                {study.radiologist && (<p className="text-xs text-gray-600">
      ...
      {/* Detail Modal */}
      {selectedStudy && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              ...
              <button onClick={closeDetailView} aria-label="Cerrar detalles" className="text-gray-400 hover:text-gray-600 text-2xl">
                ×
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              {/* Abnormal Flag */}
              {selectedStudy.isAbnormal && (<div className="bg-orange-50 border-l-4 border-orange-500 p-4 rounded">
                  <p className="text-sm font-semibold text-orange-800">⚠️ Hallazgos Anormales Detectados</p>
                </div>)}

              {/* Images Gallery */}
              {selectedStudy.imageUrls.length > 0 && (<div>
                  <h4 className="text-sm font-semibold text-gray-700 mb-3">Imágenes ({selectedStudy.imageUrls.length})</h4>
                  <div className="grid grid-cols-3 gap-3">
                    {selectedStudy.imageUrls.map((url, idx) => (<div key={idx} onClick={() => openImageViewer(url)} className="relative h-32 bg-gray-100 rounded-lg overflow-hidden cursor-pointer hover:opacity-75 transition-opacity">
              ...
              {/* Findings & Impression */}
              {(selectedStudy.findings || selectedStudy.impression) && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {selectedStudy.findings && (<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  {selectedStudy.impression && (<div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedStudy.accessionNumber && (<div>
                ...
                {selectedStudy.studyInstanceUID && (<div className="col-span-2">
                ...
                {selectedStudy.indication && (<div className="col-span-2">
                ...
                {selectedStudy.orderingDoctor && (<div>
                ...
                {selectedStudy.performingFacility && (<div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">
                    {(0, date_fns_1.format)(new Date(selectedStudy.studyDate), "d 'de' MMMM, yyyy HH:mm", { locale: locale_1.es })}
                  </p>
                </div>

                {selectedStudy.reportDate && (<div>
                    <h4 className="text-xs font-semibold text-gray-500 uppercase mb-1">Fecha del Reporte</h4>
                    <p className="text-sm text-gray-900">
                      {(0, date_fns_1.format)(new Date(selectedStudy.reportDate), "d 'de' MMMM, yyyy HH:mm", { locale: locale_1.es })}
                    </p>
                  </div>)}
              </div>

              {/* Notes */}
              {selectedStudy.notes && (<div>
              ...
              {/* Report URL */}
              {selectedStudy.reportUrl && (<div>
                  <h4 className="text-sm font-semibold text-gray-700 mb-2">Reporte</h4>
                  <a href={selectedStudy.reportUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
            ...
            <div className="sticky bottom-0 bg-gray-50 border-t border-gray-200 px-6 py-4 flex justify-end">
              <button onClick={closeDetailView} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors font-medium">
                Cerrar
              </button>
            </div>
          </div>
      ...
      {/* Image Viewer Modal */}
      {viewingImage && (<div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center p-4 z-[60]" onClick={closeImageViewer}>
          <button onClick={closeImageViewer} aria-label="Cerrar visor de imagen" className="absolute top-4 right-4 text-white text-4xl hover:text-gray-300">
            ×
          </button>
          <img src={viewingImage} alt="Imagen ampliada" className="max-w-full max-h-full object-contain" onClick={(e) => e.stopPropagation()}/>
        </div>)}
    </div>);
}
//# sourceMappingURL=ImagingStudiesList.js.map