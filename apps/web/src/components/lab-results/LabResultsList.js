...
exports.default = LabResultsList;
...
function LabResultsList({ patientId }) {
    ...
    const [criticalFilter, setCriticalFilter] = (0, react_1.useState)('ALL');
    ...
    const fetchLabResults = async () => {
        try {
            ...
            if (criticalFilter !== 'ALL')
                params.append('isCritical', criticalFilter);
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select value={criticalFilter} onChange={(e) => setCriticalFilter(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              <option value="ALL">Todos</option>
      ...
      {/* Results Table */}
      {labResults.length === 0 ? (<div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {labResults.map((result) => (<tr key={result.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => handleResultClick(result)}>
                    <td className="px-6 py-4">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{result.testName}</div>
                        {result.testCode && (<div className="text-xs text-gray-500">LOINC: {result.testCode}</div>)}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {(0, date_fns_1.format)(new Date(result.resultDate), 'd MMM yyyy', { locale: locale_1.es })}
                      </div>
                      <div className="text-xs text-gray-500">
                        {(0, date_fns_1.format)(new Date(result.resultDate), 'HH:mm', { locale: locale_1.es })}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {result.isCritical && (<span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-red-600 text-white">
                            CRÍTICO
                          </span>)}
                        {result.isAbnormal && !result.isCritical && (<span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800 border border-orange-300">
                            Anormal
                          </span>)}
                      </div>
                    </td>
      ...
      {/* Detail Modal */}
      {selectedResult && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              {/* Flags */}
              {(selectedResult.isCritical || selectedResult.isAbnormal) && (<div className="flex items-center gap-3">
                  {selectedResult.isCritical && (<span className="inline-flex items-center px-3 py-1 rounded-lg text-sm font-bold bg-red-600 text-white">
                      ⚠️ RESULTADO CRÍTICO
                    </span>)}
                  {selectedResult.isAbnormal && !selectedResult.isCritical && (<span className="inline-flex items-center px-3 py-1 rounded-lg text-sm font-medium bg-orange-100 text-orange-800 border border-orange-300">
                      Resultado Anormal
                    </span>)}
                </div>)}
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                {selectedResult.referenceRange && (<div className="text-sm text-blue-700 mt-2">
                    Rango de referencia: {selectedResult.referenceRange}
                  </div>)}
              </div>

              {/* Interpretation */}
              {selectedResult.interpretation && (<div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedResult.orderingDoctor && (<div>
                ...
                {selectedResult.performingLab && (<div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">
                    {(0, date_fns_1.format)(new Date(selectedResult.resultDate), "d 'de' MMMM, yyyy HH:mm", { locale: locale_1.es })}
                  </p>
                </div>

                {selectedResult.reviewedDate && (<div>
                    <h4 className="text-xs font-semibold text-gray-500 uppercase mb-1">Fecha de Revisión</h4>
                    <p className="text-sm text-gray-900">
                      {(0, date_fns_1.format)(new Date(selectedResult.reviewedDate), "d 'de' MMMM, yyyy HH:mm", { locale: locale_1.es })}
                    </p>
                  </div>)}
              </div>

              {/* Notes */}
              {selectedResult.notes && (<div>
              ...
              {/* Attachment */}
              {selectedResult.attachmentUrl && (<div>
                  <h4 className="text-sm font-semibold text-gray-700 mb-2">Adjunto</h4>
                  <a href={selectedResult.attachmentUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                    📄 Ver Documento
                  </a>
                </div>)}
            </div>
...
//# sourceMappingURL=LabResultsList.js.map