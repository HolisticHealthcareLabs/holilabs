 ...
 * Shows side-by-side comparison of two clinical note versions
...
function computeDiff(oldText: string | null, newText: string | null): DiffSegment[] {
...
export default function VersionDiffViewer({ oldVersion, newVersion }: VersionDiffViewerProps) {
  const sections = [
  ...
  const changedSections = sections.filter(section =>
    oldVersion.changedFields?.includes(section.key)
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-red-50 border-2 border-red-200 rounded-lg p-4">
          ...
          <div className="text-sm text-red-700">
            ...
            <p><strong>Fecha:</strong> {format(new Date(oldVersion.createdAt), "d 'de' MMMM, yyyy HH:mm", { locale: es })}</p>
          </div>
        </div>
        ...
        <div className="bg-green-50 border-2 border-green-200 rounded-lg p-4">
          ...
          <div className="text-sm text-green-700">
            ...
            <p><strong>Fecha:</strong> {format(new Date(newVersion.createdAt), "d 'de' MMMM, yyyy HH:mm", { locale: es })}</p>
          </div>
        </div>
      </div>
      ...
      {changedSections.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {changedSections.map(section => (
            <span
              key={section.key}
              className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 border border-yellow-300"
            >
              {section.label}
            </span>
          ))}
      ...
      <div className="space-y-6">
        {sections.map(section => {
          ...
          return (
            <div key={section.key} className={`border-2 rounded-lg overflow-hidden ${
              isChanged ? 'border-yellow-300 bg-yellow-50' : 'border-gray-200 bg-white'
            }`}>
              {/* Section Header */}
              <div className={`px-4 py-2 font-bold text-sm ${
                ...
                {section.label}
                {isChanged && (
                  <span className="ml-2 text-xs font-normal">(modificado)</span>
                )}
              </div>
                    ...
                    <div className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">
                      {diff
                        ...
                        .map((seg, idx) => (
                          <div
                            key={idx}
                            className={seg.type === 'removed' ? 'bg-red-200 text-red-900 px-1 rounded' : ''}
                          >
                            {seg.text}
                          </div>
                        ))}
                    ...
                    <div className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">
                      {diff
                        ...
                        .map((seg, idx) => (
                          <div
                            key={idx}
                            className={seg.type === 'added' ? 'bg-green-200 text-green-900 px-1 rounded' : ''}
                          >
                            {seg.text}
                          </div>
                        ))}
