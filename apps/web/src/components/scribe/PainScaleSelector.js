...
exports.default = PainScaleSelector;
const react_1 = require("react");
const LanguageContext_1 = require("@/contexts/LanguageContext");
function PainScaleSelector({ onSelectPainScore, patientId, className = '', }) {
    const { t } = (0, LanguageContext_1.useLanguage)();
    const [selectedScore, setSelectedScore] = (0, react_1.useState)(null);
    const [isCreatingAssessment, setIsCreatingAssessment] = (0, react_1.useState)(false);
    const [assessmentSuccess, setAssessmentSuccess] = (0, react_1.useState)(false);
    ...
    const createPainAssessment = async (score, description) => {
        if (!patientId)
            return;
    ...
    const handleSelectScore = async (scoreData) => {
    ...
    return (<div className={`bg-gradient-to-br from-blue-50 to-purple-50 border-2 border-blue-200 rounded-xl p-4 ${className}`}>
      <div className="mb-4">
      ...
      <div className="grid grid-cols-5 sm:grid-cols-6 lg:grid-cols-11 gap-2">
        {painScale.map((item) => (<button key={item.score} onClick={() => handleSelectScore(item)} className={`p-3 bg-gradient-to-br ${item.color} text-white rounded-lg hover:shadow-lg hover:scale-110 transition-all flex flex-col items-center space-y-1 border-2 ${selectedScore === item.score ? item.borderColor + ' ring-4 ring-blue-300 scale-105' : 'border-transparent'}`} title={`${item.score}/10 - ${item.description}`}>
      ...
      {selectedScore !== null && (<div className="mt-4 p-3 bg-white border-2 border-blue-300 rounded-lg">
      ...
      {isCreatingAssessment && (<div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div className="flex items-center space-x-2">
      ...
      {assessmentSuccess && !isCreatingAssessment && (<div className="mt-4 p-3 bg-green-50 border-2 border-green-300 rounded-lg animate-pulse">
          <div className="flex items-center space-x-2">
...
//# sourceMappingURL=PainScaleSelector.js.map