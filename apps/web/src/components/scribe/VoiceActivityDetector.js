...
exports.default = VoiceActivityDetector;
const react_1 = require("react");
function VoiceActivityDetector({ stream, isRecording, onVoiceActivity, onSilenceDetected, silenceThresholdMs = 3000, // 3 seconds default
...
className = '', }) {
    ...
    const [silenceDuration, setSilenceDuration] = (0, react_1.useState)(0);
    ...
    const silenceStartRef = (0, react_1.useRef)(null);
    const lastVoiceTimeRef = (0, react_1.useRef)(Date.now());
    (0, react_1.useEffect)(() => {
    ...
    const getVolumeColor = () => {
        if (currentVolume > volumeThreshold + 50)
            return 'bg-green-500';
    ...
    if (!isRecording)
        return null;
    return (<div className={`bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl p-4 ${className}`}>
      <div className="flex items-center justify-between">
        {/* Voice Activity Indicator */}
        <div className="flex items-center space-x-3">
          <div className="relative">
            {/* Pulsing ring when voice active */}
            {isVoiceActive && (<div className="absolute inset-0 bg-green-400 rounded-full animate-ping opacity-75"/>)}
            {/* Static circle */}
            <div className={`w-4 h-4 rounded-full transition-all duration-200 ${isVoiceActive ? 'bg-green-500 scale-110' : 'bg-gray-400 scale-100'}`}/>
          </div>
          <div>
        ...
        <div className="flex items-center space-x-1">
          {getVolumeBars().map((filled, i) => (<div key={i} className={`w-1.5 rounded-full transition-all duration-100 ${filled ? getVolumeColor() : 'bg-gray-200'}`} style={{ height: `${8 + i * 2}px` }}/>))}
        </div>
      </div>

      {/* Silence Warning */}
      {silenceDuration > silenceThresholdMs - 1000 && silenceDuration < silenceThresholdMs && (<div className="mt-3 flex items-center space-x-2 text-orange-600">
      ...
      {process.env.NODE_ENV === 'development' && (<div className="mt-2 text-xs text-gray-500 font-mono">
