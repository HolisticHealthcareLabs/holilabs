...
interface Intervention {
  ...
  text: string;
  icon: string;
  category: 'pain' | 'comfort' | 'family' | 'spiritual' | 'symptom';
  color: string;
}

interface QuickInterventionsPanelProps {
  onInsertText: (text: string) => void;
  className?: string;
}

export default function QuickInterventionsPanel({
  onInsertText,
  className = '',
  ...
  return (
    <div className={`bg-gradient-to-br from-purple-50 to-blue-50 border-2 border-purple-200 rounded-xl p-4 ${className}`}>
      <div className="mb-3">
      ...
      <div className="space-y-4">
        {Object.entries(groupedInterventions).map(([category, categoryInterventions]) => (
          <div key={category}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
              {categoryInterventions.map((intervention) => (
                <button
                  key={intervention.id}
                  onClick={() => onInsertText(intervention.text)}
                  className={`px-3 py-2 bg-gradient-to-r ${intervention.color} text-white rounded-lg hover:shadow-lg hover:scale-105 transition-all font-semibold text-sm flex flex-col items-center space-y-1`}
