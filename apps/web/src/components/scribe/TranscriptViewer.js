...
function TranscriptViewer({ segments, onSegmentCorrect, readonly = false }) {
    ...
    const handleSaveEdit = (index, originalText) => {
    ...
    return (<div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
      {segments.map((segment, idx) => {
            const isLowConfidence = segment.confidence < 0.85;
            const isEditing = editingIndex === idx;
            const isHovered = hoveredIndex === idx;
            return (<div key={idx} onMouseEnter={() => setHoveredIndex(idx)} onMouseLeave={() => setHoveredIndex(null)} className={`relative group p-4 rounded-lg border-2 transition-all ${isEditing
            ...
            {isLowConfidence && !isEditing && (<div className="absolute -top-2 left-4 bg-yellow-500 text-white text-xs px-3 py-1 rounded-full font-bold shadow-md flex items-center gap-1">
                <span>⚠️</span>
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-3">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${segment.speaker === 'Doctor'
              ...
              <div className="flex items-center gap-2">
                {/* Confidence Badge */}
                <span className={`text-xs px-3 py-1 rounded-full font-bold border-2 ${getConfidenceColor(segment.confidence)}`} title={getConfidenceBadge(segment.confidence)}>
                  {Math.round(segment.confidence * 100)}%
                </span>

                {/* Edit Button */}
                {!readonly && !isEditing && isHovered && (<button onClick={() => handleStartEdit(idx, segment.text)} className="text-xs px-3 py-1 bg-blue-600 text-white rounded-full font-medium hover:bg-blue-700 transition-colors flex items-center gap-1">
            ...
            {isEditing ? (<div className="space-y-3">
                <textarea value={editText} onChange={(e) => setEditText(e.target.value)} className="w-full p-3 border-2 border-blue-400 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 leading-relaxed min-h-[100px] resize-y" autoFocus onKeyDown={(e) => {
                        if (e.key === 'Escape')
                            handleCancelEdit();
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={handleCancelEdit} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border-2 border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                      Cancelar
                    </button>
                    <button onClick={() => handleSaveEdit(idx, segment.text)} className="px-4 py-2 text-sm font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors shadow-md flex items-center gap-2">
                ...
                {isLowConfidence && (<div className="mt-3 p-2 bg-yellow-100 border-l-4 border-yellow-500 rounded text-xs">
