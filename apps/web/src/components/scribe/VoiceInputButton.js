...
exports.default = VoiceInputButton;
const react_1 = require("react");
const LanguageContext_1 = require("@/contexts/LanguageContext");
function VoiceInputButton({ onTranscript, language = 'pt-BR', className = '', }) {
    ...
    const [isSupported, setIsSupported] = (0, react_1.useState)(true);
    ...
    if (!isSupported) {
        ...
        <button disabled className="px-4 py-2 bg-gray-200 text-gray-500 rounded-lg cursor-not-allowed flex items-center space-x-2" title={t('soapTemplates.voiceInput.unsupportedBrowser')}>
          <span>🎤</span>
    ...
    return (<div className={`flex flex-col space-y-2 ${className}`}>
      <div className="flex items-center space-x-2">
        <button onClick={handleClick} disabled={isProcessing} className={`px-4 py-2 rounded-lg font-semibold transition-all shadow-md flex items-center space-x-2 ${isListening
            ? 'bg-red-500 text-white hover:bg-red-600 animate-pulse'
            : 'bg-gradient-to-r from-blue-500 to-purple-600 text-white hover:from-blue-600 hover:to-purple-700'} disabled:opacity-50 disabled:cursor-not-allowed`} title={isListening
        ...
        {isListening && (<div className="flex space-x-1">
      ...
      {(isListening || isProcessing) && transcript && (<div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
      ...
      {/* Error Display */}
      {error && (<div className="p-2 bg-red-50 border border-red-200 rounded-lg">
          <span className="text-xs text-red-600">{error}</span>
        </div>)}
    </div>);
}
