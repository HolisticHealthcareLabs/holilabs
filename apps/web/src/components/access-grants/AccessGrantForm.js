...
exports.default = AccessGrantForm;
...
function AccessGrantForm({ patientId, onSuccess, onCancel }) {
    ...
    const [imagingStudies, setImagingStudies] = (0, react_1.useState)([]);
    const [loadingResources, setLoadingResources] = (0, react_1.useState)(false);
    ...
    const fetchLabResults = async () => {
        try {
            ...
            if (response.ok) {
                ...
                setLabResults(data.data.map((result) => ({
                    id: result.id,
    ...
    const fetchImagingStudies = async () => {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                setImagingStudies(data.data.map((study) => ({
                    id: study.id,
    ...
    const handleExpirationTypeChange = (type) => {
        setFormData((prev) => ({ ...prev, expirationType: type }));
        const now = new Date();
        let expiresAt = '';
        switch (type) {
            case '1week':
    ...
    return (<form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center">
                <input type="radio" name="grantedToType" value="EXTERNAL" checked={formData.grantedToType === 'EXTERNAL'} onChange={handleChange} className="h-4 w-4 text-blue-600 focus:ring-blue-500"/>
                <span className="ml-2 text-sm text-gray-900">Usuario Externo</span>
              </label>
              <label className="flex items-center">
                <input type="radio" name="grantedToType" value="USER" checked={formData.grantedToType === 'USER'} onChange={handleChange} className="h-4 w-4 text-blue-600 focus:ring-blue-500"/>
          ...
          {formData.grantedToType === 'EXTERNAL' ? (<>
              <div>
                ...
                <input type="text" name="grantedToName" value={formData.grantedToName} onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Dr. Juan Pérez"/>
              </div>
              ...
              <div>
                ...
                <input type="email" name="grantedToEmail" value={formData.grantedToEmail} onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="doctor@ejemplo.com"/>
              </div>
            </>) : (<div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Seleccionar Usuario <span className="text-red-600">*</span>
              </label>
              <select name="grantedToId" value={formData.grantedToId} onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <select name="resourceType" value={formData.resourceType} onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
          ...
          {formData.resourceType === 'LAB_RESULT' && (<div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Seleccionar Resultado de Laboratorio
              </label>
              {loadingResources ? (<p className="text-sm text-gray-500">Cargando resultados...</p>) : (<select name="resourceId" value={formData.resourceId} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                  <option value="">Seleccione un resultado...</option>
                  {labResults.map((result) => (<option key={result.id} value={result.id}>
                      {result.name} ({new Date(result.date).toLocaleDateString()})
                    </option>))}
                </select>)}
            </div>)}

          {formData.resourceType === 'IMAGING_STUDY' && (<div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Seleccionar Estudio de Imágenes
              </label>
              {loadingResources ? (<p className="text-sm text-gray-500">Cargando estudios...</p>) : (<select name="resourceId" value={formData.resourceId} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                  <option value="">Seleccione un estudio...</option>
                  {imagingStudies.map((study) => (<option key={study.id} value={study.id}>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-3">
          <label className="flex items-center">
            <input type="checkbox" name="canView" checked={formData.canView} onChange={handleChange} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
            <span className="ml-3">
          ...
          <label className="flex items-center">
            <input type="checkbox" name="canDownload" checked={formData.canDownload} onChange={handleChange} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
            <span className="ml-3">
          ...
          <label className="flex items-center">
            <input type="checkbox" name="canShare" checked={formData.canShare} onChange={handleChange} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
            <span className="ml-3">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
        ...
        ].map((option) => (<button key={option.value} type="button" onClick={() => handleExpirationTypeChange(option.value)} className={`px-3 py-2 text-sm font-medium rounded-lg border transition-colors ${formData.expirationType === option.value
          ...
          {formData.expirationType === 'custom' && (<div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Fecha y Hora de Vencimiento
              </label>
              <input type="datetime-local" name="expiresAt" value={formData.expiresAt} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"/>
            </div>)}
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div>
          ...
          <textarea name="purpose" value={formData.purpose} onChange={handleChange} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Ej: Segunda opinión médica, consulta con especialista, trámite de seguro..."/>
        </div>
      </div>
...
//# sourceMappingURL=AccessGrantForm.js.map