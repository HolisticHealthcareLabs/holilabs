...
export default function AccessGrantsList({ patientId, onGrantCreated }: AccessGrantsListProps) {
  ...
  const getGrantStatusColor = (grant: AccessGrant) => {
    if (grant.isRevoked) return statusColors.revoked;
    if (grant.isExpired) return statusColors.expired;
    return statusColors.active;
  };

  const getGrantStatusLabel = (grant: AccessGrant) => {
    if (grant.isRevoked) return statusLabels.revoked;
    if (grant.isExpired) return statusLabels.expired;
    return statusLabels.active;
  };

  const getRecipientDisplay = (grant: AccessGrant) => {
  ...
  const getResourceDisplay = (grant: AccessGrant) => {
    if (grant.labResult) {
      return `${grant.labResult.testName} (${format(new Date(grant.labResult.resultDate), 'd MMM yyyy', { locale: es })})`;
    }
    if (grant.imagingStudy) {
      return `${grant.imagingStudy.modality}: ${grant.imagingStudy.description} (${format(new Date(grant.imagingStudy.studyDate), 'd MMM yyyy', { locale: es })})`;
    }
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900">
                      {grant.expiresAt
                        ? format(new Date(grant.expiresAt), "d 'de' MMMM, yyyy", { locale: es })
                        : 'Sin vencimiento'}
                    </p>
                  </div>
                      ...
                      <p className="text-sm text-gray-900">
                        ...
                        {grant.lastAccessedAt && (
                          <span className="text-gray-600">
                            {' · Último: '}
                            {format(new Date(grant.lastAccessedAt), 'd MMM yyyy', { locale: es })}
                          </span>
                        )}
                      </p>
                    </div>
                    ...
                    <div className="md:col-span-2">
                      <div className="bg-red-50 border border-red-200 rounded p-3">
                        <p className="text-xs font-semibold text-red-800 mb-1">
                          Revocado el {format(new Date(grant.revokedAt), "d 'de' MMMM, yyyy HH:mm", { locale: es })}
                        </p>
                        {grant.revokedReason && (
                          <p className="text-xs text-red-700">Razón: {grant.revokedReason}</p>
                        )}
                      </div>
                    </div>
                  )}
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="Ej: Ya no necesito compartir esta información"
              />
            </div>
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                onClick={() => handleRevokeGrant(revokeConfirm)}
                disabled={revoking}
