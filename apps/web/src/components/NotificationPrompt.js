...
function NotificationPrompt() {
    ...
    (0, react_1.useEffect)(() => {
        ...
        if (!push_notifications_1.pushNotifications.isSupported()) {
            return;
        }
        // Get current permission
        const currentPermission = push_notifications_1.pushNotifications.getPermission();
    ...
    const handleEnable = async () => {
        const permission = await push_notifications_1.pushNotifications.requestPermission();
        setPermission(permission);
        if (permission === 'granted') {
            // Subscribe to push notifications
            await push_notifications_1.pushNotifications.init();
            await push_notifications_1.pushNotifications.subscribe();
            setShowPrompt(false);
            // Show success notification
            await push_notifications_1.pushNotifications.showNotification({
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in zoom-in-95 duration-300">
        ...
        <div className="space-y-2">
          <button onClick={handleEnable} className="w-full px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-lg hover:from-blue-700 hover:to-blue-800 transition-all shadow-lg">
            Activar Notificaciones
          </button>

          <div className="flex space-x-2">
            <button onClick={handleRemindLater} className="flex-1 px-4 py-2 text-gray-600 hover:text-gray-800 font-medium text-sm transition">
              Más Tarde
            </button>
            <button onClick={handleDismiss} className="flex-1 px-4 py-2 text-gray-600 hover:text-gray-800 font-medium text-sm transition">
              No, Gracias
            </button>
          </div>
        </div>
...
//# sourceMappingURL=NotificationPrompt.js.map