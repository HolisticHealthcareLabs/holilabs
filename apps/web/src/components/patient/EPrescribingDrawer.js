...
exports.default = EPrescribingDrawer;
const react_1 = require("react");
function EPrescribingDrawer({ isOpen, onClose, currentMedications, patientId, clinicianId, }) {
    const [commandInput, setCommandInput] = (0, react_1.useState)('');
    const [parsedPrescription, setParsedPrescription] = (0, react_1.useState)(null);
    const [pin, setPin] = (0, react_1.useState)('');
    const [signature, setSignature] = (0, react_1.useState)('');
    const [signingMethod, setSigningMethod] = (0, react_1.useState)('pin');
      ...
      <div className="fixed right-0 top-0 h-full w-full md:w-[600px] bg-white shadow-2xl z-50 overflow-y-auto">
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={onClose} aria-label="Cerrar gestión de recetas" className="text-gray-500 hover:text-gray-700 text-2xl">
              ×
            </button>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="space-y-3">
              {currentMedications.map((med) => (<div key={med.id} className="border border-gray-200 rounded-lg p-4">
          ...
          <div className="mb-8">
            ...
            <textarea value={commandInput} onChange={(e) => handleCommandInput(e.target.value)} placeholder="Agregar nueva receta (Ej: 'Atenolol 50mg diario por 90 días')" className="w-full p-4 border-2 border-gray-300 rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/20 resize-none font-mono text-sm" rows={3}/>

            {parsedPrescription && (<div className="mt-4 p-4 bg-green-50 border-2 border-green-200 rounded-lg animate-fade-in">
          ...
          <div className="mb-6">
            ...
            <div className="flex space-x-4 mb-4">
              <button onClick={() => setSigningMethod('pin')} className={`flex-1 py-2 rounded-lg font-medium transition ${signingMethod === 'pin'
            ? 'bg-primary text-white'
            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                PIN Clínico
              </button>
              <button onClick={() => setSigningMethod('signature')} className={`flex-1 py-2 rounded-lg font-medium transition ${signingMethod === 'signature'
            ? 'bg-primary text-white'
            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                Firma Digital
              </button>
            ...
            {/* PIN Input */}
            {signingMethod === 'pin' && (<div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Ingrese su PIN Clínico (4-6 dígitos)
                </label>
                <input type="password" value={pin} onChange={(e) => setPin(e.target.value)} maxLength={6} className="w-full p-3 border-2 border-gray-300 rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/20 text-center text-2xl tracking-widest" placeholder="••••"/>
              </div>)}

            {/* Signature Pad */}
            {signingMethod === 'signature' && (<div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Dibuje su firma
                </label>
                <div className="border-2 border-gray-300 rounded-lg overflow-hidden">
                  <canvas ref={canvasRef} width={552} height={200} onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} onMouseLeave={stopDrawing} className="w-full bg-white cursor-crosshair"/>
                </div>
                <button onClick={clearSignature} className="mt-2 text-sm text-gray-600 hover:text-gray-800">
                  Limpiar firma
                </button>
              </div>)}
          </div>
          ...
          {/* Submit Button */}
          <button onClick={handleSubmit} disabled={!isValid || isSubmitting} className="w-full py-4 bg-primary text-white rounded-lg font-semibold text-lg hover:bg-primary/90 transition disabled:opacity-50 disabled:cursor-not-allowed">
...
//# sourceMappingURL=EPrescribingDrawer.js.map