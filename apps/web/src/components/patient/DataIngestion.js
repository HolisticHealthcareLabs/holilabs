...
exports.default = DataIngestion;
const react_1 = require("react");
function DataIngestion({ patientId, onContextUpdate }) {
    ...
    const processFile = async (file) => {
        ...
        for (let i = 0; i < steps.length; i++) {
            const step = steps[i];
            // Update status
            setFiles((prev) => prev.map((f) => (f.id === file.id ? { ...f, status: step, progress: ((i + 1) / steps.length) * 100 } : f)));
    ...
    return (<div>
      ...
      {/* Upload Zone */}
      <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className={`border-4 border-dashed rounded-xl p-12 text-center transition ${isDragging ? 'border-primary bg-primary/5' : 'border-gray-300 bg-gray-50'}`}>
        <input type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.docx" onChange={(e) => handleFileSelect(e.target.files)} className="hidden" id="file-upload-ingestion"/>
      ...
      {files.length > 0 && (<div className="mt-8 space-y-6">
          <h4 className="font-semibold text-gray-800 text-lg">Estado de Procesamiento</h4>

          {files.map((file) => (<div key={file.id} className="border-2 border-gray-200 rounded-lg p-6 bg-white">
              ...
              <div className="w-full bg-gray-200 rounded-full h-3 mb-4 overflow-hidden">
                <div className="bg-gradient-to-r from-primary to-green-500 h-full transition-all duration-500 ease-out" style={{ width: `${file.progress}%` }}/>
              </div>
              ...
              <div className="grid grid-cols-5 gap-2">
                {processingSteps.map((step, i) => {
                    ...
                    return (<div key={step.key} className={`p-3 rounded-lg text-center transition ${isComplete
                            ? isCurrent
              ...
              {file.status === 'deidentifying' && (<div className="mt-4 p-4 bg-yellow-50 border-2 border-yellow-300 rounded-lg animate-fade-in">
              ...
              {/* Success Message */}
              {file.status === 'synchronized' && (<div className="mt-4 p-4 bg-green-50 border-2 border-green-300 rounded-lg animate-fade-in">
                  <div className="flex items-center space-x-3">
                    <span className="text-2xl">✅</span>
                    <div>
