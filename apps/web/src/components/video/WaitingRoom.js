...
function WaitingRoom({ userName, userType, appointmentTime, onJoinCall, }) {
    const [isMuted, setIsMuted] = (0, react_1.useState)(false);
    const [isVideoOff, setIsVideoOff] = (0, react_1.useState)(false);
    const [hasPermissions, setHasPermissions] = (0, react_1.useState)(false);
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center p-6">
      <div className="max-w-5xl w-full">
        <div className="grid lg:grid-cols-2 gap-8">
          {/* Video Preview */}
          <framer_motion_1.motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="relative">
            <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
              {/* Video */}
              <div className="relative aspect-video bg-gradient-to-br from-gray-800 to-gray-900">
                {hasPermissions && !isVideoOff ? (<video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover mirror"/>) : (<div className="absolute inset-0 flex items-center justify-center">
                    {isLoading ? (<div className="text-center">
                        <framer_motion_1.motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: 'linear' }} className="w-16 h-16 border-4 border-blue-500 border-t-transparent rounded-full mx-auto mb-4"/>
                        <p className="text-white">Solicitando permisos...</p>
                      </div>) : (<div className="text-center px-6">
                        ...
                        {!hasPermissions && (<p className="text-white/70 text-sm">
              ...
              <div className="p-6 bg-white border-t border-gray-100">
                <div className="flex items-center justify-center gap-3">
                  {/* Mute Button */}
                  <framer_motion_1.motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={toggleMute} disabled={!hasPermissions} className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${isMuted
            ...
            : 'bg-gray-100 hover:bg-gray-200 text-gray-700'} disabled:opacity-50 disabled:cursor-not-allowed`}>
                  ...
                  {/* Video Button */}
                  <framer_motion_1.motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={toggleVideo} disabled={!hasPermissions} className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${isVideoOff
            ? 'bg-red-500 hover:bg-red-600 text-white'
            : 'bg-gray-100 hover:bg-gray-200 text-gray-700'} disabled:opacity-50 disabled:cursor-not-allowed`}>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* Info and Join */}
          <framer_motion_1.motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col justify-center">
            ...
            : 'Tu médico te está esperando'}
              ...
              <div className="space-y-4 mb-8">
                ...
                {appointmentTime && (<div className="flex items-start gap-3 p-4 bg-purple-50 rounded-xl">
              ...
              {/* Join Button */}
              <framer_motion_1.motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={onJoinCall} disabled={!hasPermissions} className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold py-4 px-6 rounded-xl shadow-lg shadow-blue-500/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none">
                Unirse a la consulta
              </framer_motion_1.motion.button>

              {!hasPermissions && (<p className="text-center text-sm text-red-600 mt-4">
