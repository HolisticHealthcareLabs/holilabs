...
interface PainHistoryTabProps {
  painAssessments: PainAssessment[];
  patientId: string;
  onRefresh?: () => void;
}

export default function PainHistoryTab({
  painAssessments,
  patientId,
  onRefresh,
}: PainHistoryTabProps) {
  ...
  const getPainTypeLabel = (type?: string) => {
  ...
  const getPainLevelLabel = (score: number) => {
    ...
    if (score <= 3) return 'Dolor leve';
    if (score <= 6) return 'Dolor moderado';
    if (score <= 8) return 'Dolor intenso';
    return 'Dolor severo';
  };

  const getPainLevelColor = (score: number) => {
    ...
    <div className="space-y-6">
      ...
      {painAssessments.length === 0 ? (
        ...
        <>
          {/* Chart View */}
          {view === 'chart' && (
            <PainTrendChart assessments={painAssessments} showInterventions={true} />
          )}
                ...
                return (
                  <div
                    ...
                    className={`bg-white border-2 ${
                      isExpanded ? `border-${painColor}-400` : 'border-gray-200'
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            ...
                            <div className="text-sm text-gray-600">
                              {format(assessedDate, "EEEE, dd 'de' MMMM, yyyy 'a las' HH:mm", { locale: es })}
                            </div>
                          </div>
                          {assessment.painType && (
                            <span className="px-3 py-1 bg-purple-100 border border-purple-300 text-purple-900 text-xs font-bold rounded-full">
                        ...
                        {assessment.interventionsGiven && assessment.interventionsGiven.length > 0 && (
                      ...
                      <div className="px-5 pb-5 space-y-4 border-t border-gray-200 pt-4">
                        <div className="grid md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-3">
                            ...
                            {assessment.quality && assessment.quality.length > 0 && (
                              <div>
                                <div className="text-xs font-semibold text-gray-600 mb-1">Calidad:</div>
                                <div className="flex flex-wrap gap-1">
                                  {assessment.quality.map((q, i) => (
                            ...
                            {assessment.aggravatingFactors && assessment.aggravatingFactors.length > 0 && (
                              <div>
                                <div className="text-xs font-semibold text-gray-600 mb-1">Factores Agravantes:</div>
                                <ul className="text-sm text-gray-900 list-disc list-inside">
                                  {assessment.aggravatingFactors.map((f, i) => (
                                    <li key={i}>{f}</li>
                                  ))}
                                </ul>
                              </div>
                            ...
                            {assessment.relievingFactors && assessment.relievingFactors.length > 0 && (
                              <div>
                                <div className="text-xs font-semibold text-gray-600 mb-1">Factores Aliviantes:</div>
                                <ul className="text-sm text-gray-900 list-disc list-inside">
                                  {assessment.relievingFactors.map((f, i) => (
                                    <li key={i}>{f}</li>
                                  ))}
                                </ul>
                              </div>
                        ...
                        {assessment.interventionsGiven && assessment.interventionsGiven.length > 0 && (
                          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                            ...
                            <ul className="space-y-1">
                              {assessment.interventionsGiven.map((intervention, i) => (
                            ...
                            {assessment.responseToTreatment && (
