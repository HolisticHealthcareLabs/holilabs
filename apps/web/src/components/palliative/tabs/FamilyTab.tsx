...
interface FamilyMember {
  ...
  lastAccessedPortal?: string | Date;
...
export default function FamilyTab({
  ...
  const getRelationshipIcon = (relationship: string) => {
    ...
    if (normalized.includes('esposa') || normalized.includes('esposo') || normalized.includes('pareja')) return '💑';
    if (normalized.includes('hijo') || normalized.includes('hija')) return '👶';
    if (normalized.includes('padre') || normalized.includes('madre')) return '👴';
    if (normalized.includes('hermano') || normalized.includes('hermana')) return '👫';
    return '👤';
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        {communicationPreferences && (
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 border-2 border-blue-300 rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              ...
              {communicationPreferences.interpreterNeeded !== undefined && (
                <div>
                  <div className="text-xs font-semibold text-blue-700 uppercase mb-1">Intérprete Requerido</div>
                  <div className={`text-base font-semibold ${
                    communicationPreferences.interpreterNeeded ? 'text-orange-600' : 'text-green-600'
                  }`}>
        ...
        {spiritualPreferences && (
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 border-2 border-purple-300 rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              ...
              {spiritualPreferences.chaplainRequested !== undefined && (
                <div>
                  <div className="text-xs font-semibold text-purple-700 uppercase mb-1">Capellán Solicitado</div>
                  <div className={`text-base font-semibold ${
                    spiritualPreferences.chaplainRequested ? 'text-purple-600' : 'text-gray-600'
                  }`}>
              ...
              {spiritualPreferences.specialRituals && spiritualPreferences.specialRituals.length > 0 && (
                <div>
                  <div className="text-xs font-semibold text-purple-700 uppercase mb-1">Rituales Especiales</div>
                  <div className="flex flex-wrap gap-1">
                    {spiritualPreferences.specialRituals.map((ritual, i) => (
                      <span key={i} className="px-2 py-1 bg-white border border-purple-300 text-purple-900 text-xs rounded-full">
                        {ritual}
                      </span>
                    ))}
                  </div>
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h4 className="text-lg font-bold text-gray-900 flex items-center space-x-2">
                          ...
                          {member.isPrimaryContact && (
                            <span className="px-2 py-1 bg-green-100 border border-green-300 text-green-900 text-xs font-bold rounded-full">
                              ⭐ Contacto Principal
                            </span>
                          )}
                        </h4>
                      ...
                      <div className="flex items-center space-x-2">
                        {member.canMakeMedicalDecisions && (
                          <span className="px-2 py-1 bg-orange-100 border border-orange-300 text-orange-900 text-xs font-bold rounded-full">
                            🏥 Decisiones Médicas
                          </span>
                        )}
                        {member.portalAccess && (
                          <span className={`px-2 py-1 bg-${accessColor}-100 border border-${accessColor}-300 text-${accessColor}-900 text-xs font-bold rounded-full`}>
                            🔑 Portal
                          </span>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {member.notificationPreferences.email && (
                            <span className="px-3 py-1 bg-blue-100 text-blue-900 text-xs rounded-full flex items-center space-x-1">
                              <span>📧</span>
                              <span>Email activado</span>
                            </span>
                          )}
                          {member.notificationPreferences.sms && (
                            <span className="px-3 py-1 bg-green-100 text-green-900 text-xs rounded-full flex items-center space-x-1">
                              <span>💬</span>
                              <span>SMS activado</span>
                            </span>
                          )}
                          {member.notificationPreferences.whatsapp && (
                            <span className="px-3 py-1 bg-green-100 text-green-900 text-xs rounded-full flex items-center space-x-1">
                              <span>📱</span>
                    ...
                    {member.portalAccess && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        ...
                        <div className="space-y-2">
                          ...
                          {member.lastAccessedPortal && (
                            <div>
                              <div className="text-xs font-semibold text-blue-800 mb-1">Último Acceso:</div>
                              <div className="text-sm text-blue-900">
                                {format(
                                  typeof member.lastAccessedPortal === 'string'
                                    ? parseISO(member.lastAccessedPortal)
                                    : member.lastAccessedPortal,
