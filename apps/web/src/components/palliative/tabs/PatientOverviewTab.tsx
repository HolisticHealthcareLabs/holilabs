...
interface PatientOverviewTabProps {
  patient: PatientData;
  latestPainAssessment?: PainAssessment;
  latestQoLAssessment?: QoLAssessment;
...
export default function PatientOverviewTab({
  ...
  const dob = typeof patient.dateOfBirth === 'string' ? parseISO(patient.dateOfBirth) : patient.dateOfBirth;
  ...
  return (
    <div className="space-y-6">
          ...
          {latestPainAssessment ? (
            <>
              ...
              <div className={`text-xs text-${painColor}-600 mt-2`}>
                Evaluado: {format(
                  typeof latestPainAssessment.assessedAt === 'string'
                    ? parseISO(latestPainAssessment.assessedAt)
                    : latestPainAssessment.assessedAt,
                  "dd/MM 'a las' HH:mm",
                  { locale: es }
                )}
              </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 border-2 border-purple-300 rounded-xl p-5 shadow-md hover:shadow-lg transition-shadow">
          ...
          {activeCarePlans.length > 0 ? (
            <div className="text-xs text-purple-700 space-y-1">
              ...
              {activeCarePlans.length > 2 && (
                <div className="font-semibold">+{activeCarePlans.length - 2} más</div>
              )}
            </div>
          ) : (
            <div className="text-purple-600 text-xs">Sin planes activos</div>
          )}
        </div>
      </div>
      ...
      {patient.isPalliativeCare && (
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 border-2 border-purple-300 rounded-xl shadow-sm p-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-4">
            ...
            {patient.diagnosisDate && (
              <div>
                ...
                <div className="text-base font-semibold text-gray-900">
                  {format(
                    typeof patient.diagnosisDate === 'string' ? parseISO(patient.diagnosisDate) : patient.diagnosisDate,
                    "dd/MM/yyyy",
                    { locale: es }
                  )}
                </div>
            ...
            {patient.karnofskyScore !== undefined && (
              <div>
          ...
          <div className="mt-4 pt-4 border-t border-purple-200">
            <div className="flex flex-wrap gap-2">
              {patient.hasDNR && (
                <div className="px-3 py-1 bg-red-100 border-2 border-red-500 text-red-900 rounded-full text-xs font-bold flex items-center space-x-1">
                  <span>⛔</span>
              ...
              {patient.hasAdvanceDirective && (
                <div className="px-3 py-1 bg-blue-100 border-2 border-blue-500 text-blue-900 rounded-full text-xs font-bold flex items-center space-x-1">
                  <span>📄</span>
          ...
          {patient.assignedClinician && (
            <div className="flex items-center space-x-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <div>
                ...
                {patient.assignedClinician.specialty && (
                  <div className="text-xs text-gray-600">{patient.assignedClinician.specialty}</div>
                )}
              </div>
            </div>
          )}
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-3">
          <Link
            href={`/dashboard/scribe?patientId=${patient.id}`}
            className="px-4 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg hover:shadow-lg transition-all font-semibold text-center"
          >
            📝 Nueva Nota SOAP
          </Link>
