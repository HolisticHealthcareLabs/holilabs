...
exports.default = CarePlansTab;
const react_1 = require("react");
const date_fns_1 = require("date-fns");
const locale_1 = require("date-fns/locale");
function CarePlansTab({ carePlans, patientId, onRefresh, }) {
    const [selectedStatus, setSelectedStatus] = (0, react_1.useState)('all');
    const [selectedCategory, setSelectedCategory] = (0, react_1.useState)('all');
    const [expandedPlan, setExpandedPlan] = (0, react_1.useState)(null);
    // Filter care plans
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        ...
        <div className="flex flex-wrap gap-3">
          <div>
            ...
            <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
          ...
          <div>
            ...
            <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
      ...
      {/* Care Plans List */}
      {filteredPlans.length === 0 ? (<div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl p-12 text-center">
          ...
          {filteredPlans.map((plan) => {
                ...
                return (<div key={plan.id} className={`bg-white border-2 ${isExpanded ? `border-${statusColor}-400` : 'border-gray-200'} rounded-xl shadow-sm hover:shadow-md transition-all overflow-hidden`}>
                {/* Plan Header */}
                <button onClick={() => setExpandedPlan(isExpanded ? null : plan.id)} className="w-full p-5 text-left flex items-start space-x-4 hover:bg-gray-50 transition-colors">
                  {/* Category Icon */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {plan.description && (<p className="text-sm text-gray-700 line-clamp-2">{plan.description}</p>)}

                    <div className="flex items-center space-x-4 text-xs text-gray-500 mt-2">
                      <span>🎯 {plan.goals.length} objetivo(s)</span>
                      {plan.targetDate && (<span>
                          📅 Meta:{' '}
                          {(0, date_fns_1.format)(typeof plan.targetDate === 'string'
                            ? (0, date_fns_1.parseISO)(plan.targetDate)
                            : plan.targetDate, 'dd/MM/yyyy', { locale: locale_1.es })}
                        </span>)}
                      {plan.assignedTeam && plan.assignedTeam.length > 0 && (<span>👥 {plan.assignedTeam.length} miembro(s) del equipo</span>)}
                    </div>
                  </div>
                ...
                {isExpanded && (<div className="px-5 pb-5 space-y-4 border-t border-gray-200 pt-4">
                    {/* Goals */}
                    {plan.goals.length > 0 && (<div>
                        <h5 className="text-sm font-bold text-gray-900 mb-2">🎯 Objetivos del Plan:</h5>
                        <ul className="space-y-2">
                          {plan.goals.map((goal, index) => (<li key={index} className="flex items-start space-x-2 text-sm">
                    ...
                    {/* Progress Notes */}
                    {plan.progressNotes && plan.progressNotes.length > 0 && (<div>
                        <h5 className="text-sm font-bold text-gray-900 mb-2">📝 Notas de Progreso:</h5>
                        <div className="space-y-2">
                          {plan.progressNotes.map((note, index) => (<div key={index} className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-gray-700">
                              {note}
                            </div>))}
                        </div>
                      </div>)}
                    ...
                    <div className="flex items-center space-x-6 text-xs text-gray-500 pt-3 border-t border-gray-200">
                      <span>
                        Creado:{' '}
                        {(0, date_fns_1.format)(typeof plan.createdAt === 'string'
                            ? (0, date_fns_1.parseISO)(plan.createdAt)
                            : plan.createdAt, "dd/MM/yyyy 'a las' HH:mm", { locale: locale_1.es })}
                      </span>
                      <span>
                        Actualizado:{' '}
                        {(0, date_fns_1.format)(typeof plan.updatedAt === 'string'
                            ? (0, date_fns_1.parseISO)(plan.updatedAt)
                            : plan.updatedAt, "dd/MM/yyyy 'a las' HH:mm", { locale: locale_1.es })}
                      </span>
                    </div>
                    ...
                    <div className="flex space-x-3 pt-3">
                      ...
                      {plan.status === 'ACTIVE' && (<button className="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors text-sm font-semibold">
                          ✓ Marcar Completado
                        </button>)}
                    </div>
                  </div>)}
      ...
      {/* Summary Statistics */}
      {carePlans.length > 0 && (<div className="bg-gradient-to-r from-purple-50 to-blue-50 border-2 border-purple-300 rounded-xl p-6">
...
//# sourceMappingURL=CarePlansTab.js.map