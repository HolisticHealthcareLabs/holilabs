...
const ShareRecordModal_1 = __importDefault(require("@/components/portal/ShareRecordModal"));
function RecordDetailPage({ params }) {
    ...
    const [isShareModalOpen, setIsShareModalOpen] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
    ...
    if (error || !record) {
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
          ...
          <link_1.default href="/portal/records" className="inline-block bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-6 rounded-lg transition-colors">
            Volver a registros
          </link_1.default>
        </div>
      </div>);
    ...
    return (<div className="p-4 lg:p-8 max-w-5xl mx-auto">
      {/* Back Button */}
      <link_1.default href="/portal/records" className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-6 transition-colors">
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 mb-6">
        <div className="flex items-start justify-between mb-6">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            {record.signedAt && (<span className="text-xs text-gray-500">
                Firmado el {(0, date_fns_1.format)(new Date(record.signedAt), 'dd/MM/yyyy')}
              </span>)}
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
          ...
          <div>
            ...
            {record.clinician.licenseNumber && (<p className="text-xs text-gray-500">Cédula: {record.clinician.licenseNumber}</p>)}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <button onClick={() => window.print()} className="flex items-center gap-2 px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-900 rounded-lg font-semibold transition-colors">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button onClick={() => setIsShareModalOpen(true)} className="flex items-center gap-2 px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold transition-colors">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button onClick={handleDownloadPDF} disabled={isPdfLoading} className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed text-white rounded-lg font-semibold transition-colors">
          {isPdfLoading ? (<>
      ...
      {/* Share Modal */}
      <ShareRecordModal_1.default isOpen={isShareModalOpen} onClose={() => setIsShareModalOpen(false)} recordId={params.id} recordTitle={record.chiefComplaint || 'Consulta Médica'}/>
    </div>);
}
