...
function AppointmentsPage() {
    ...
    const [summary, setSummary] = (0, react_1.useState)({ total: 0, upcoming: 0, past: 0 });
    const [showPast, setShowPast] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
        fetchAppointments();
    }, []);
    ...
    const getTimeUntil = (startTime) => {
        ...
        if (diffDays > 7) {
            return (0, date_fns_1.format)(start, "d 'de' MMMM", { locale: locale_1.es });
        }
        else if (diffDays > 0) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleNewAppointment} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all shadow-lg hover:shadow-xl">
        ...
        {error && (<div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center mb-6">
            ...
            <button onClick={fetchAppointments} className="px-4 py-2 bg-red-100 text-red-800 rounded-lg hover:bg-red-200 transition-colors">
              Reintentar
            </button>
          </div>)}
        ...
        <div className="mb-8">
          ...
          {upcomingAppointments.length === 0 ? (<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
              <outline_1.CalendarIcon className="h-16 w-16 text-gray-400 mx-auto mb-4"/>
              ...
              <button onClick={handleNewAppointment} className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                <outline_1.PlusIcon className="h-5 w-5"/>
                Agendar Cita
              </button>
              ...
              {upcomingAppointments.map((appointment) => {
                ...
                return (<div key={appointment.id} onClick={() => handleAppointmentClick(appointment.id)} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md hover:border-blue-300 transition-all cursor-pointer group">
                    {/* Header */}
                    ...
                    <div className="flex items-center gap-4 mb-4 text-sm text-gray-600">
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          {(0, date_fns_1.format)(new Date(appointment.startTime), "EEEE, d 'de' MMMM", {
                        locale: locale_1.es,
                    })}
                        </span>
                      </div>
        ...
        {pastAppointments.length > 0 && (<div>
            <button onClick={() => setShowPast(!showPast)} className="flex items-center gap-2 text-lg font-bold text-gray-900 mb-4 hover:text-blue-600 transition-colors">
              <span>Citas Anteriores ({pastAppointments.length})</span>
              <svg className={`h-5 w-5 transition-transform ${showPast ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/>
              </svg>
            </button>

            {showPast && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pastAppointments.map((appointment) => {
                    const TypeIcon = typeIcons[appointment.type];
                    return (<div key={appointment.id} onClick={() => handleAppointmentClick(appointment.id)} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md hover:border-gray-300 transition-all cursor-pointer group opacity-75 hover:opacity-100">
                      {/* Header */}
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-4 mb-4 text-sm text-gray-600">
                        <div className="flex items-center gap-1">
                          <outline_1.CalendarIcon className="h-4 w-4"/>
                          <span>
                            {(0, date_fns_1.format)(new Date(appointment.startTime), "d 'de' MMMM, yyyy", {
                            locale: locale_1.es,
                        })}
                          </span>
                        </div>
