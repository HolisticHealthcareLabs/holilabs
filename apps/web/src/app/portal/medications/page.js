...
function MedicationsPage() {
    ...
    const [activeTab, setActiveTab] = (0, react_1.useState)('active');
    const [selectedMed, setSelectedMed] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        fetchMedications();
    }, []);
    ...
    const getDaysUntilEnd = (endDate) => {
        return (0, date_fns_1.differenceInDays)(new Date(endDate), new Date());
    };
    ...
    if (isLoading) {
        ...
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid gap-4">
            {[1, 2, 3].map((i) => (<div key={i} className="bg-white rounded-xl p-6 h-40"/>))}
          </div>
        </div>
      </div>);
    }
    if (error) {
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
          ...
          <button onClick={fetchMedications} className="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-6 rounded-lg transition-colors">
            Reintentar
          </button>
        </div>
      </div>);
    ...
    return (<div className="p-4 lg:p-8 max-w-6xl mx-auto">
      ...
      {/* Refill Alert */}
      {needsRefill.length > 0 && (<framer_motion_1.motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="bg-yellow-50 border border-yellow-200 rounded-xl p-6 mb-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-2">
                {needsRefill.map((med) => (<div key={med.id} className="flex items-center gap-2 text-sm text-yellow-900">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-xl p-6 text-white shadow-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-2xl">🔄</span>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-gradient-to-br from-purple-400 to-purple-600 rounded-xl p-6 text-white shadow-lg">
          <div className="flex items-center justify-between mb-2">
      ...
      <div className="flex gap-2 mb-6 border-b border-gray-200">
        <button onClick={() => setActiveTab('active')} className={`px-6 py-3 font-semibold transition-all ${activeTab === 'active'
            ? 'text-green-600 border-b-2 border-green-600'
            : 'text-gray-600 hover:text-gray-900'}`}>
          Activos ({activeMedications.length})
        </button>
        <button onClick={() => setActiveTab('inactive')} className={`px-6 py-3 font-semibold transition-all ${activeTab === 'inactive'
            ? 'text-green-600 border-b-2 border-green-600'
            : 'text-gray-600 hover:text-gray-900'}`}>
      ...
      <framer_motion_1.AnimatePresence mode="wait">
        {displayMedications.length === 0 ? (<framer_motion_1.motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            {displayMedications.map((medication, index) => {
                ...
                return (<framer_motion_1.motion.div key={medication.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} onClick={() => setSelectedMed(medication)} className="bg-white rounded-xl shadow-md border border-gray-200 p-6 hover:shadow-lg transition-all cursor-pointer">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        {medication.genericName && (<p className="text-sm text-gray-600 mb-2">
                    ...
                    {needsRefillAlert && (<span className="px-3 py-1 bg-yellow-100 text-yellow-700 text-xs font-semibold rounded-full">
                        ⚠️ Renovar pronto
                      </span>)}
                  </div>

                  {/* Instructions */}
                  {medication.instructions && (<div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                  ...
                  {/* Prescription Info */}
                  {medication.prescription && (<div className="border-t border-gray-200 pt-4">
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <p className="text-gray-500 mb-1">Inicio</p>
                          <p className="font-semibold text-gray-900">
                            {(0, date_fns_1.format)(new Date(medication.prescription.startDate), 'dd/MMM/yyyy', {
                            locale: locale_1.es,
                        })}
                          </p>
                        </div>
                        <div>
                          ...
                          <p className={`font-semibold ${needsRefillAlert ? 'text-yellow-600' : 'text-gray-900'}`}>
                            {(0, date_fns_1.format)(new Date(medication.prescription.endDate), 'dd/MMM/yyyy', {
                            locale: locale_1.es,
                        })}
                            {daysUntilEnd !== null && (<span className="text-xs ml-2">({daysUntilEnd}d)</span>)}
                          </p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <span className={`inline-block px-2 py-1 rounded text-xs font-semibold ${medication.prescription.status === 'ACTIVE'
                  ...
                  {medication.sideEffects && (<div className="mt-4 bg-red-50 border border-red-200 rounded-lg p-3">
