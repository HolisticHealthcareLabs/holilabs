...
exports.default = ConsultationsPage;
...
function ConsultationsPage() {
    ...
    if (error) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
          ...
          <button onClick={() => fetchRecordings()} className="w-full px-6 py-3 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all">
            Reintentar
          </button>
        </div>
      </div>);
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Recordings List */}
        {recordings.length === 0 ? (<div className="bg-white rounded-2xl shadow-md border border-gray-200 p-12 text-center">
            ...
            {recordings.map((recording, index) => (<framer_motion_1.motion.div key={recording.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="bg-white rounded-xl shadow-md border border-gray-200 p-6 hover:shadow-lg transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600 mb-3">
                        ...
                        {recording.audioDuration && (<span className="flex items-center gap-1">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {/* Action */}
                  {recording.status === 'COMPLETED' && (<link_1.default href={`/portal/consultations/${recording.id}`} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium text-sm transition-colors flex items-center gap-2">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
