...
exports.default = AIScribePage;
const react_1 = require("react");
const client_1 = require("@/lib/supabase/client");
const TranscriptViewer_1 = __importDefault(require("@/components/scribe/TranscriptViewer"));
const SOAPNoteEditor_1 = __importDefault(require("@/components/scribe/SOAPNoteEditor"));
const AudioWaveform_1 = __importDefault(require("@/components/scribe/AudioWaveform"));
const VoiceActivityDetector_1 = __importDefault(require("@/components/scribe/VoiceActivityDetector"));
exports.dynamic = 'force-dynamic';
function AIScribePage() {
    ...
    const [transcriptSegments, setTranscriptSegments] = (0, react_1.useState)([]);
    const [soapNote, setSoapNote] = (0, react_1.useState)(null);
    const [sessionId, setSessionId] = (0, react_1.useState)(null);
    const [searchQuery, setSearchQuery] = (0, react_1.useState)('');
    const [audioStream, setAudioStream] = (0, react_1.useState)(null);
    const [smartAutoPauseEnabled, setSmartAutoPauseEnabled] = (0, react_1.useState)(true);
    const [isVoiceActive, setIsVoiceActive] = (0, react_1.useState)(false);
    const [showAudioSourceModal, setShowAudioSourceModal] = (0, react_1.useState)(false);
    const [audioSource, setAudioSource] = (0, react_1.useState)('microphone');
    ...
    const filteredPatients = patients.filter((p) => p.firstName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const handleSilenceDetected = (silenceDurationMs) => {
    ...
    const handleSegmentCorrect = async (index, newText, originalText) => {
        if (!sessionId)
            return;
        try {
    ...
    return (<div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 lg:p-8">
      {/* Header */}
      <div className="mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div className="p-4">
              <input type="text" placeholder="Buscar paciente..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"/>
              <div className="mt-4 space-y-2 max-h-[600px] overflow-y-auto">
                {filteredPatients.map((patient) => (<button key={patient.id} onClick={() => setSelectedPatient(patient)} disabled={recordingState !== 'idle'} className={`w-full text-left p-3 rounded-lg border transition-all ${selectedPatient?.id === patient.id
                ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500 shadow-sm'
                : 'bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-500'} ${recordingState !== 'idle' ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
                    <div className="font-semibold text-gray-900 dark:text-white">
                      {patient.firstName} {patient.lastName}
                    </div>
                    <div className="text-sm text-gray-500 dark:text-gray-400">MRN: {patient.mrn}</div>
                  </button>))}
                {filteredPatients.length === 0 && (<div className="text-center text-gray-500 dark:text-gray-400 py-8">
        ...
        <div className="lg:col-span-5">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div className="p-6">
              ...
              {selectedPatient && (<div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-700">
              ...
              <div className="text-center mb-6">
                <div className={`inline-flex items-center space-x-2 px-6 py-3 rounded-full ${recordingState === 'recording'
            ...
            : recordingState === 'paused'
                ...
                {(recordingState === 'recording' || recordingState === 'paused') && (<div className="mt-4 text-4xl font-mono font-bold text-gray-900 dark:text-white">
              ...
              <div className="mb-4">
                <AudioWaveform_1.default stream={audioStream} isRecording={recordingState === 'recording'} className="h-32"/>
              </div>

              {/* Voice Activity Detection (Nuance DAX-style) */}
              <div className="mb-6">
                <VoiceActivityDetector_1.default stream={audioStream} isRecording={recordingState === 'recording'} onVoiceActivity={handleVoiceActivity} onSilenceDetected={handleSilenceDetected} silenceThresholdMs={smartAutoPauseEnabled ? 5000 : Infinity} volumeThreshold={30}/>
              </div>

              {/* Smart Auto-Pause Toggle */}
              {(recordingState === 'recording' || recordingState === 'paused') && (<div className="mb-6 flex items-center justify-between bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 p-4">
                  ...
                  <button onClick={() => setSmartAutoPauseEnabled(!smartAutoPauseEnabled)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${smartAutoPauseEnabled ? 'bg-green-600' : 'bg-gray-300 dark:bg-gray-600'}`}>
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${smartAutoPauseEnabled ? 'translate-x-6' : 'translate-x-1'}`}/>
                  </button>
                </div>)}

              {/* Recording Controls */}
              <div className="flex items-center justify-center space-x-4 mb-6">
                {recordingState === 'idle' && (<button onClick={handleStartRecordingClick} disabled={!selectedPatient} className="flex items-center space-x-2 px-8 py-4 bg-gradient-to-r from-red-500 to-pink-600 text-white font-bold rounded-full hover:from-red-600 hover:to-pink-700 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                    <span className="text-2xl">🎙️</span>
                    <span>Iniciar Grabación</span>
                  </button>)}
                {recordingState === 'recording' && (<>
                    <button onClick={pauseRecording} className="flex items-center space-x-2 px-6 py-3 bg-yellow-500 text-white font-bold rounded-full hover:bg-yellow-600 transition-all shadow-lg">
                      <span className="text-xl">⏸</span>
                      <span>Pausar</span>
                    </button>
                    <button onClick={stopRecording} className="flex items-center space-x-2 px-6 py-3 bg-red-600 text-white font-bold rounded-full hover:bg-red-700 transition-all shadow-lg">
                      <span className="text-xl">⏹</span>
                      <span>Finalizar</span>
                    </button>
                  </>)}
                {recordingState === 'paused' && (<>
                    <button onClick={resumeRecording} className="flex items-center space-x-2 px-6 py-3 bg-green-500 text-white font-bold rounded-full hover:bg-green-600 transition-all shadow-lg">
                ...
                {recordingState === 'completed' && (<button onClick={resetRecording} className="flex items-center space-x-2 px-6 py-3 bg-blue-600 text-white font-bold rounded-full hover:bg-blue-700 transition-all shadow-lg">
                    <span className="text-xl">🔄</span>
              ...
              {transcriptSegments.length > 0 && (<div className="border-t border-gray-200 dark:border-gray-700 pt-6">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center">
                    <span className="mr-2">🎙️</span>
                    Transcripción con Identificación de Hablantes
                  </h3>
                  <TranscriptViewer_1.default segments={transcriptSegments} onSegmentCorrect={handleSegmentCorrect} readonly={recordingState !== 'completed'}/>
                </div>)}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div className="p-6 max-h-[800px] overflow-y-auto">
              {!soapNote ? (<div className="text-center text-gray-500 dark:text-gray-400 py-12">
                ...
                </div>) : (<SOAPNoteEditor_1.default note={soapNote} onSave={handleSaveNote} onSign={handleSignNote} onNotifyPatient={handleNotifyPatient}/>)}
            </div>
          </div>
        </div>
      </div>
      ...
      {showAudioSourceModal && (<div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center p-4 border-2 border-gray-200 dark:border-gray-600 rounded-lg cursor-pointer transition-all hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-blue-300 dark:hover:border-blue-500">
                <input type="radio" name="audioSource" value="microphone" checked={audioSource === 'microphone'} onChange={(e) => setAudioSource(e.target.value)} className="w-5 h-5 text-blue-600 focus:ring-blue-500"/>
              ...
              {/* System Audio Option */}
              <label className="flex items-center p-4 border-2 border-gray-200 dark:border-gray-600 rounded-lg cursor-pointer transition-all hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-blue-300 dark:hover:border-blue-500">
                <input type="radio" name="audioSource" value="system" checked={audioSource === 'system'} onChange={(e) => setAudioSource(e.target.value)} className="w-5 h-5 text-blue-600 focus:ring-blue-500"/>
                <div className="ml-4 flex items-center flex-1">
                  <span className="text-3xl mr-3">💻</span>
                  <div>
              ...
              {/* Both Option */}
              <label className="flex items-center p-4 border-2 border-gray-200 dark:border-gray-600 rounded-lg cursor-pointer transition-all hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-blue-300 dark:hover:border-blue-500">
                <input type="radio" name="audioSource" value="both" checked={audioSource === 'both'} onChange={(e) => setAudioSource(e.target.value)} className="w-5 h-5 text-blue-600 focus:ring-blue-500"/>
                <div className="ml-4 flex items-center flex-1">
                  <span className="text-3xl mr-3">🎧</span>
                  <div>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowAudioSourceModal(false)} className="flex-1 px-4 py-3 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-lg font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 transition-all">
                Cancelar
              </button>
              <button onClick={startRecording} className="flex-1 px-4 py-3 text-white bg-gradient-to-r from-red-500 to-pink-600 rounded-lg font-semibold hover:from-red-600 hover:to-pink-700 transition-all shadow-lg">
                Iniciar Grabación
              </button>
            </div>
          </div>
