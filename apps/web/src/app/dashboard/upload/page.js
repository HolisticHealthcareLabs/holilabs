...
const PatientSearch_1 = __importDefault(require("@/components/PatientSearch"));
...
function UploadPage() {
    ...
    const [isNewPatient, setIsNewPatient] = (0, react_1.useState)(false);
    const [uploadedFile, setUploadedFile] = (0, react_1.useState)(null);
    const [newPatientData, setNewPatientData] = (0, react_1.useState)({
        firstName: '',
        lastName: '',
        birthYear: '',
        condition: '',
    ...
    return (<DashboardLayout_1.default>
      <div className="p-4 md:p-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
          ...
          <div className="px-6 py-4">
            <div className="flex items-center justify-between max-w-2xl mx-auto">
              <div className={`flex items-center ${step === 'select' ? 'text-primary font-bold' : 'text-gray-400 dark:text-gray-500'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-2 ${step === 'select' ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>
                  1
                </div>
              ...
              <div className={`flex items-center ${step === 'upload' || step === 'assign' || step === 'confirm' ? 'text-primary font-bold' : 'text-gray-400 dark:text-gray-500'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-2 ${step === 'upload' || step === 'assign' || step === 'confirm' ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>
                  2
                </div>
              ...
              <div className={`flex items-center ${step === 'assign' || step === 'confirm' ? 'text-primary font-bold' : 'text-gray-400 dark:text-gray-500'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-2 ${step === 'assign' || step === 'confirm' ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>
                  3
                </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {step === 'select' && (<div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 border border-gray-200 dark:border-gray-700">
              ...
              <div className="space-y-6">
                ...
                <PatientSearch_1.default onSelectPatient={handleSelectPatient} showMostViewed={true}/>
              </div>
            </div>)}

          {/* Step 2: Upload Document */}
          {step === 'upload' && (<div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 border border-gray-200 dark:border-gray-700">
              ...
              {isNewPatient && (<div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/50 rounded-lg">
                  <h3 className="font-bold mb-3 text-gray-900 dark:text-white">Datos del Nuevo Paciente</h3>
                  <div className="grid md:grid-cols-2 gap-4">
                    <input type="text" placeholder="Nombre" value={newPatientData.firstName} onChange={(e) => setNewPatientData({ ...newPatientData, firstName: e.target.value })} className="px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded"/>
                    <input type="text" placeholder="Apellido" value={newPatientData.lastName} onChange={(e) => setNewPatientData({ ...newPatientData, lastName: e.target.value })} className="px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded"/>
                    <input type="text" placeholder="Año de Nacimiento" value={newPatientData.birthYear} onChange={(e) => setNewPatientData({ ...newPatientData, birthYear: e.target.value })} className="px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded"/>
                    <input type="text" placeholder="Condición Principal" value={newPatientData.condition} onChange={(e) => setNewPatientData({ ...newPatientData, condition: e.target.value })} className="px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded"/>
                  </div>
                </div>)}

              {/* DocuSign-style Drop Zone */}
              <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className={`border-4 border-dashed rounded-lg p-12 text-center transition ${isDragging ? 'border-primary bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600 hover:border-primary hover:bg-gray-50 dark:hover:bg-gray-700/50'}`}>
                ...
                <label className="inline-block px-6 py-3 bg-primary text-white rounded-lg cursor-pointer hover:bg-primary/90">
                  <input type="file" onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0])} className="hidden" accept=".pdf,.jpg,.png,.dicom,.csv"/>
                  Seleccionar Archivo
                </label>
              ...
              {uploadedFile && (<div className="mt-6 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700/50 rounded-lg">
              ...
              <div className="mt-6 flex space-x-2">
                <button onClick={() => setStep('select')} className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-900 dark:text-white">
                  ← Atrás
                </button>
              </div>
            </div>)}
          ...
          {step === 'assign' && (<div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 border border-gray-200 dark:border-gray-700">
              ...
              <div className="space-y-6">
                ...
                <div className="flex space-x-2">
                  <button onClick={() => setStep('upload')} className="flex-1 px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-900 dark:text-white">
                    ← Atrás
                  </button>
                  <button onClick={handleAssignToWallet} className="flex-1 px-4 py-3 bg-primary text-white rounded-lg hover:bg-primary/90 font-bold">
          ...
          {/* Step 4: Confirmation */}
          {step === 'confirm' && (<div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 text-center border border-gray-200 dark:border-gray-700">
              <div className="text-6xl mb-4">⏳</div>
