...
exports.default = PreventionPage;
const react_1 = require("react");
const DashboardTile_1 = __importDefault(require("@/components/dashboard/DashboardTile"));
...
function PreventionPage() {
    const [selectedProtocol, setSelectedProtocol] = (0, react_1.useState)(null);
    const [activeTab, setActiveTab] = (0, react_1.useState)('interventions');
    const [selectedSpecialty, setSelectedSpecialty] = (0, react_1.useState)('all');
    const specialties = Array.from(new Set(protocolTemplates.map((p) => p.specialty)));
    const filteredProtocols = selectedSpecialty === 'all'
        ? protocolTemplates
    ...
    return (<div className="min-h-screen bg-gray-50 p-4 lg:p-8">
      ...
      <div className="mb-6">
        ...
        <select value={selectedSpecialty} onChange={(e) => setSelectedSpecialty(e.target.value)} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {filteredProtocols.map((protocol) => (<DashboardTile_1.default key={protocol.id} title={protocol.name} description={protocol.description} icon={protocol.icon} href="#" color={protocol.color} chartEmoji="📊" badge={protocol.specialty} onClick={() => setSelectedProtocol(protocol)}/>))}
      </div>

      {/* Protocol Details */}
      <div>
          {!selectedProtocol ? (<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
              ...
              <div className="border-b border-gray-200">
                <div className="flex">
                  <button onClick={() => setActiveTab('interventions')} className={`px-6 py-3 font-semibold border-b-2 transition-colors ${activeTab === 'interventions'
                ? 'border-blue-500 text-blue-600'
                : 'border-transparent text-gray-600 hover:text-gray-900'}`}>
                    Intervenciones
                  </button>
                  <button onClick={() => setActiveTab('screenings')} className={`px-6 py-3 font-semibold border-b-2 transition-colors ${activeTab === 'screenings'
                ? 'border-blue-500 text-blue-600'
                : 'border-transparent text-gray-600 hover:text-gray-900'}`}>
                    Tamizajes
                  </button>
                  <button onClick={() => setActiveTab('education')} className={`px-6 py-3 font-semibold border-b-2 transition-colors ${activeTab === 'education'
                ? 'border-blue-500 text-blue-600'
                : 'border-transparent text-gray-600 hover:text-gray-900'}`}>
                    Educación
                  </button>
              ...
              <div className="p-6">
                {activeTab === 'interventions' && (<div>
                    ...
                    <div className="space-y-3">
                      {selectedProtocol.interventions.map((intervention, index) => (<div key={index} className="flex items-start p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                {activeTab === 'screenings' && (<div>
                    <h3 className="text-lg font-bold text-gray-900 mb-4">
                      Tamizajes y Estudios de Gabinete
                    </h3>
                    <div className="space-y-3">
                      {selectedProtocol.screenings.map((screening, index) => (<div key={index} className="flex items-start p-4 bg-purple-50 rounded-lg border border-purple-200">
                ...
                {activeTab === 'education' && (<div>
                    ...
                    <div className="space-y-3">
                      {selectedProtocol.education.map((topic, index) => (<div key={index} className="flex items-start p-4 bg-green-50 rounded-lg border border-green-200">
