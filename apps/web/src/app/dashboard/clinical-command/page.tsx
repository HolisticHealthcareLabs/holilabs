...
import { Stethoscope, Star, User, X, Eye, Upload, Mail, CheckCircle2, Activity, Save } from 'lucide-react';
...
export default function ClinicalCommandCenterPage() {
  ...
  const locale = language; // compatibility with components expecting 'locale'
  ...
  function handleReviewComplete(result?: any) {
    setIsReviewModalOpen(false);
    setIsBillingModalOpen(true);

    // Fire-and-forget: persist review to summaryDraft
    if (encounterIdRef.current && result) {
        ...
        <>
          ...
          <Suspense fallback={null}>
            <FinalizeConsultationModal
              ...
              onFinalize={handleBillingComplete}
            ...
            <div className="mx-sm flex items-center gap-sm dark:bg-amber-500/10 border dark:border-amber-500/30 px-md py-xs" style={{ borderRadius: 'var(--radius-xl)', backgroundColor: 'var(--surface-warning)', borderColor: 'var(--border-default)' }}>
              ...
              <div className="relative flex items-center gap-md shrink-0">
                ...
                {preAuthPopoverOpen && (
                  <Suspense fallback={null}>
                    <PreAuthSendPopover
                      open={preAuthPopoverOpen}
                      patientId={selectedPatient.id}
                      onClose={() => setPreAuthPopoverOpen(false)}
                      onSend={() => setHasPreAuth(true)}
                    />
                  </Suspense>
                )}
              </div>
                  ...
                  <div className="px-[var(--space-md)] py-[var(--space-sm)] border-t border-[var(--border-default)]">
                    <AlertBanner
                      alerts={liveAlerts.map(a => ({
                        ruleId: a.ruleId,
                        severity: a.severity === 'critical' || a.severity === 'severe' ? 'RED' : a.severity === 'moderate' ? 'AMBER' : 'GREEN',
                        message: `${a.name}: ${a.recommendation}`
                      }))}
                      patientId={selectedPatient.id}
