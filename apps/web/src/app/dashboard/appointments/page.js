...
function AppointmentsContent() {
    ...
    const [showAppleModal, setShowAppleModal] = (0, react_1.useState)(false);
    const [appleCredentials, setAppleCredentials] = (0, react_1.useState)({ appleId: '', appPassword: '' });
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 py-8">
      <div className="container mx-auto px-6 max-w-5xl">
        {/* Header */}
        <div className="mb-8">
          <div className="flex items-center space-x-3 mb-2">
            <link_1.default href="/dashboard" className="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
              ← Regresar
            </link_1.default>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {integrations.map((integration) => {
            ...
            return (<div key={integration.provider} className="bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-lg transition-all border border-gray-200 dark:border-gray-700 overflow-hidden">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        {integration.connected && (<p className="text-xs text-green-600 dark:text-green-400 font-medium">✓ Conectado</p>)}
                      </div>
                    </div>
                  </div>

                  {integration.connected ? (<div className="space-y-3">
                      ...
                      <button onClick={() => disconnectCalendar(integration.provider)} disabled={isProcessing} className="w-full px-4 py-2 text-sm font-medium text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors disabled:opacity-50">
                        {isProcessing ? 'Desconectando...' : 'Desconectar'}
                      </button>
                    </div>) : (<button onClick={() => connectCalendar(integration.provider)} disabled={isProcessing || loading} className={`w-full px-6 py-3 font-semibold text-white rounded-lg transition-all ${isProcessing || loading
                      ...
                      {isProcessing ? (<span className="flex items-center justify-center space-x-2">
                        ...
                        </span>) : (`Conectar con ${getProviderName(integration.provider)}`)}
                    </button>)}
                </div>
              </div>);
        })}
        ...
        {showAppleModal && (<div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="email" value={appleCredentials.appleId} onChange={(e) => setAppleCredentials({ ...appleCredentials, appleId: e.target.value })} placeholder="tu@icloud.com" className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Contraseña específica de app
                  </label>
                  <input type="password" value={appleCredentials.appPassword} onChange={(e) => setAppleCredentials({ ...appleCredentials, appPassword: e.target.value })} placeholder="xxxx-xxxx-xxxx-xxxx" className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"/>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    <a href="https://support.apple.com/es-es/HT204397" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">
              ...
              <div className="flex space-x-3 mt-6">
                ...
                <button onClick={connectAppleCalendar} disabled={!appleCredentials.appleId || !appleCredentials.appPassword} className="flex-1 px-4 py-2 bg-gradient-to-r from-gray-700 to-gray-900 text-white rounded-lg hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                  Conectar
                </button>
              </div>
            </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            ¿Necesitas ayuda? {' '}
            <link_1.default href="/dashboard/settings" className="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium">
...
function AppointmentsPage() {
    return (<react_1.Suspense fallback={<div className="p-6">Cargando...</div>}>
      <AppointmentsContent />
    </react_1.Suspense>);
}
