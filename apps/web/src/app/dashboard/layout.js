...
const NotificationPrompt_1 = __importDefault(require("@/components/NotificationPrompt"));
...
const useSessionTimeout_1 = require("@/hooks/useSessionTimeout");
const SessionTimeoutWarning_1 = require("@/components/SessionTimeoutWarning");
function DashboardContent({ children }) {
    const pathname = (0, navigation_1.usePathname)();
    const router = (0, navigation_1.useRouter)();
    const [user, setUser] = (0, react_1.useState)(null);
    const [sidebarOpen, setSidebarOpen] = (0, react_1.useState)(false);
    const { locale, t } = (0, LanguageContext_1.useLanguage)();
    const { theme } = (0, ThemeContext_1.useTheme)();
    // Session timeout (15 min idle for HIPAA compliance)
    const { showWarning, timeRemaining, extendSession, logout } = (0, useSessionTimeout_1.useSessionTimeout)({
    ...
    return (<div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          {/* Logo */}
          <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200 dark:border-gray-700">
            <link_1.default href="/dashboard" className="flex items-center space-x-2">
              <image_1.default src={theme === 'dark' ? "/logos/Logo 1_Dark (1).svg" : "/logos/Logo 1_Light.svg"} alt="Holi Labs" width={32} height={32}/>
              <span className="font-bold text-xl text-gray-900 dark:text-white">
                Holi Labs
              </span>
            </link_1.default>
            <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
          ...
          <nav className="flex-1 px-3 py-4 space-y-3 overflow-y-auto">
            ...
            return (<div key={item.href} className="relative">
                  <link_1.default href={item.href} onClick={() => setSidebarOpen(false)} className={`group relative flex items-center justify-center w-14 h-14 rounded-full transition-all duration-500 ${isActive
                    ...
                    : 'bg-white/80 dark:bg-gray-800/50 backdrop-blur-lg border border-gray-200/50 dark:border-gray-700/50 hover:scale-110 hover:shadow-xl hover:shadow-blue-500/30 dark:hover:shadow-blue-400/20'}`} style={{
                    transitionDelay: `${index * 30}ms`
                }}>
                    <span className={`text-2xl transition-transform duration-300 group-hover:scale-125 ${isActive ? 'filter drop-shadow-lg' : ''}`}>
                      {item.emoji}
                    </span>
                    ...
                    <div className="absolute left-20 top-1/2 -translate-y-1/2 pointer-events-none z-50">
                      <div className={`
                        ...
                        whitespace-nowrap
                      `}>
                        <p className={`font-semibold text-sm tracking-wide ${isActive
                    ...
                    {isActive && (<div className="absolute inset-0 rounded-full bg-gradient-to-br from-blue-400 to-purple-600 opacity-20 animate-pulse"/>)}

                    {item.badge && (<span className="absolute -top-1 -right-1 flex items-center justify-center min-w-[1.25rem] h-5 px-1.5 text-xs font-bold text-white bg-red-500 rounded-full shadow-lg z-10 animate-bounce">
                        {item.badge}
                      </span>)}
                  </link_1.default>
                </div>);
            ...
            <div className="relative" onMouseEnter={() => setExpandedGroup('clinical-tools')} onMouseLeave={() => setExpandedGroup(null)}>
              <button className="group relative flex items-center justify-center w-14 h-14 rounded-full transition-all duration-500 bg-gradient-to-br from-green-500 to-teal-600 hover:scale-110 hover:shadow-xl hover:shadow-green-500/30 dark:hover:shadow-green-400/20">
              ...
              {expandedGroup === 'clinical-tools' && (<div className="absolute left-20 top-0 z-40 flex flex-col gap-2 pl-6">
                  {clinicalToolsGroup.children.map((child, idx) => {
                const isActive = pathname === child.href || pathname.startsWith(child.href + '/');
                return (<div key={child.href} className="relative" style={{
                        ...
                        <link_1.default href={child.href} onClick={() => setSidebarOpen(false)} className={`group/child flex items-center justify-center w-10 h-10 rounded-full transition-all duration-300 ${isActive
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 p-4">
            ...
            <button onClick={handleSignOut} className="w-full flex items-center justify-center space-x-2 px-4 py-2 text-sm font-medium text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors">
      ...
      <div className="lg:pl-64">
        ...
        <header className="lg:hidden bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-30">
          <div className="flex items-center justify-between h-16 px-4">
            <button onClick={() => setSidebarOpen(true)} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center gap-2">
              ...
              <LanguageSelector_1.default currentLocale={locale}/>
      ...
      <NotificationPrompt_1.default />

      {/* Session Timeout Warning Modal */}
      <SessionTimeoutWarning_1.SessionTimeoutWarning isOpen={showWarning} timeRemaining={timeRemaining} onExtend={extendSession} onLogout={logout}/>
    </div>);
}
