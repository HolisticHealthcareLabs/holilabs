...
exports.default = SentFormsPage;
const react_1 = require("react");
const link_1 = __importDefault(require("next/link"));
function SentFormsPage() {
    const [sentForms, setSentForms] = (0, react_1.useState)([]);
    ...
    const fetchSentForms = async () => {
        try {
            ...
            if (!response.ok)
                throw new Error('Failed to fetch sent forms');
    ...
    const getStatusBadge = (status) => {
        ...
        return (<span className={`inline-block px-3 py-1 rounded-full text-xs font-medium border ${badge.className}`}>
        {badge.label}
      </span>);
    };
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <link_1.default href="/dashboard/forms" className="text-gray-400 hover:text-gray-600 transition-colors">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        ...
        ].map((status) => (<button key={status.value} onClick={() => setFilter(status.value)} className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${filter === status.value
      ...
      {/* Table */}
      {filteredForms.length === 0 ? (<div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
          ...
          <link_1.default href="/dashboard/forms" className="mt-6 inline-block px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium">
            Enviar Formulario
          </link_1.default>
        </div>) : (<div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredForms.map((form) => (<tr key={form.id} className="hover:bg-gray-50 transition-colors">
                ...
                }} className="text-blue-600 hover:text-blue-800 transition-colors font-medium" title="Copiar enlace">
                          🔗
                        </button>
                        {(form.status === 'SIGNED' || form.status === 'COMPLETED') && (<link_1.default href={`/dashboard/forms/responses/${form.id}`} className="text-green-600 hover:text-green-800 transition-colors font-medium" title="Ver respuestas">
                            👁️
                          </link_1.default>)}
                      </div>
                    </td>
