...
exports.POST = (0, middleware_1.createProtectedRoute)(async (request, context) => {
    try {
        ...
        if (!note.patient.phone) {
            return server_1.NextResponse.json({ error: 'Patient phone number not found. Cannot send WhatsApp notification.' }, { status: 400 });
        }
        ...
        const result = await (0, whatsapp_1.notifyPatientSOAPReady)({
        ...
        return server_1.NextResponse.json({ error: 'Failed to send notification', message: error.message }, { status: 500 });
    }
});
