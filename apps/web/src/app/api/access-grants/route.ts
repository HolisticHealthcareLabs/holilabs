...
export const GET = createProtectedRoute(
  async (request: NextRequest, context: any) => {
    try {
      ...
      const accessGrants = await prisma.dataAccessGrant.findMany({
        where,
        include: {
          patient: {
            select: {
  ...
  async (request: NextRequest, context: any) => {
    try {
      ...
      if (!patientId || !grantedToType || !resourceType) {
        return NextResponse.json(
          { error: 'Missing required fields: patientId, grantedToType, resourceType' },
          { status: 400 }
        );
      }
      ...
      if (grantedToType === 'USER' && !grantedToId) {
        return NextResponse.json(
          { error: 'grantedToId is required for USER type' },
          { status: 400 }
        );
      }
      ...
      if (grantedToType === 'EXTERNAL' && (!grantedToEmail || !grantedToName)) {
        return NextResponse.json(
          { error: 'grantedToEmail and grantedToName are required for EXTERNAL type' },
          { status: 400 }
        );
      }
      ...
      if (resourceType === 'LAB_RESULT' && labResultId) {
        ...
        if (!labResult || labResult.patientId !== patientId) {
          return NextResponse.json(
            { error: 'Lab result not found or does not belong to patient' },
            { status: 404 }
          );
        }
      }
      ...
      if (resourceType === 'IMAGING_STUDY' && imagingStudyId) {
        ...
        if (!imagingStudy || imagingStudy.patientId !== patientId) {
          return NextResponse.json(
            { error: 'Imaging study not found or does not belong to patient' },
            { status: 404 }
          );
        }
      }
      ...
      if (existingGrant) {
        return NextResponse.json(
          { error: 'Active grant already exists for this resource and recipient' },
          { status: 409 }
        );
      }
