...
exports.GET = (0, middleware_1.createProtectedRoute)(async (request, context) => {
    try {
        ...
        const accessGrant = await prisma_1.prisma.dataAccessGrant.findUnique({
            where: { id: grantId },
            include: {
                patient: {
                    select: {
        ...
        if (!accessGrant) {
            return server_1.NextResponse.json({ error: 'Access grant not found' }, { status: 404 });
        }
        // Compute status
        const now = new Date();
        ...
        return server_1.NextResponse.json({ error: 'Failed to fetch access grant', message: error.message }, { status: 500 });
    }
}, {
...
exports.PATCH = (0, middleware_1.createProtectedRoute)(async (request, context) => {
    try {
        ...
        const existing = await prisma_1.prisma.dataAccessGrant.findUnique({
            where: { id: grantId },
        });
        if (!existing) {
            return server_1.NextResponse.json({ error: 'Access grant not found' }, { status: 404 });
        ...
        if (existing.revokedAt && body.revoke) {
            return server_1.NextResponse.json({ error: 'Access grant is already revoked' }, { status: 400 });
        }
        const { revoke, revokedReason, expiresAt, canView, canDownload, canShare, purpose, } = body;
        // Prepare update data
        const updateData = {};
        // Handle revocation
        if (revoke === true) {
        ...
        if (!revoke) {
            if (canView !== undefined)
                updateData.canView = canView;
            if (canDownload !== undefined)
                updateData.canDownload = canDownload;
            if (canShare !== undefined)
                updateData.canShare = canShare;
            if (purpose !== undefined)
                updateData.purpose = purpose;
        }
        // Update the grant
        const updatedGrant = await prisma_1.prisma.dataAccessGrant.update({
            where: { id: grantId },
            data: updateData,
            include: {
                patient: {
        ...
        return server_1.NextResponse.json({ error: 'Failed to update access grant', message: error.message }, { status: 500 });
    }
}, {
...
exports.DELETE = (0, middleware_1.createProtectedRoute)(async (request, context) => {
    try {
        ...
        // Delete the grant
        await prisma_1.prisma.dataAccessGrant.delete({
            where: { id: grantId },
        });
        // Create audit log
        await prisma_1.prisma.auditLog.create({
        ...
        return server_1.NextResponse.json({ error: 'Failed to delete access grant', message: error.message }, { status: 500 });
    }
}, {
    roles: ['ADMIN'],
