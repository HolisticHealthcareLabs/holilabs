...
async function POST(request) {
    try {
        ...
        if (!patientId || !templateId) {
            return server_1.NextResponse.json({ error: 'Patient ID and Template ID are required' }, { status: 400 });
        }
        ...
        const template = await prisma_1.prisma.formTemplate.findUnique({
            where: { id: templateId },
            select: { id: true, title: true, structure: true },
        });
        if (!template) {
            return server_1.NextResponse.json({ error: 'Form template not found' }, { status: 404 });
        }
        ...
        const formInstance = await prisma_1.prisma.formInstance.create({
            data: {
                patientId,
                templateId,
        ...
        await prisma_1.prisma.formTemplate.update({
            where: { id: templateId },
        ...
        try {
            await (0, email_1.sendFormNotificationEmail)(patient.email, `${patient.firstName} ${patient.lastName}`, template.title, publicUrl, expiresAtDate, message || undefined, undefined // TODO: Get clinician name from session
            );
        }
        catch (emailError) {
