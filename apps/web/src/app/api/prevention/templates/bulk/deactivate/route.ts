/**
 * Bulk Deactivate Templates API
 *
 * POST /api/prevention/templates/bulk/deactivate
 * Deactivates multiple templates in a single transaction
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (templateIds.length > 100) {
      return NextResponse.json(
        { success: false, error: 'Cannot deactivate more than 100 templates at once' },
        { status: 400 }
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const auditLogs = templates.map((template) => ({
        ...
        details: `Bulk deactivated: ${template.templateName}`,
        ipAddress,
      }));
    ...
    logger.info({
      event: 'bulk_templates_deactivated',
      userId: session.user.id,
    ...
    const notification = {
      ...
      message: `${result.updated} plantilla${result.updated !== 1 ? 's' : ''} desactivada${result.updated !== 1 ? 's' : ''} exitosamente`,
    ...
    logger.error({
      event: 'bulk_deactivate_templates_error',
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to deactivate templates',
      },
      { status: 500 }
    );
  }
