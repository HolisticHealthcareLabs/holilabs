...
exports.GET = (0, middleware_1.createProtectedRoute)(async (request, context) => {
    try {
        ...
        if (!payment) {
            return server_1.NextResponse.json({ error: 'Payment not found' }, { status: 404 });
        }
        ...
        return server_1.NextResponse.json({ error: 'Failed to fetch payment', message: error.message }, { status: 500 });
    }
}, {
...
exports.PATCH = (0, middleware_1.createProtectedRoute)(async (request, context) => {
    try {
        ...
        const { status, refund, refundAmount, refundReason, notes, receiptUrl, failureReason, failureCode, } = body;
        // Prepare update data
        const updateData = {};
        // Handle refund
        if (refund === true) {
            if (existing.status !== 'COMPLETED') {
                return server_1.NextResponse.json({ error: 'Only completed payments can be refunded' }, { status: 400 });
            }
            const refundAmountValue = refundAmount || existing.amount;
            if (refundAmountValue > existing.amount) {
                return server_1.NextResponse.json({ error: 'Refund amount cannot exceed payment amount' }, { status: 400 });
            }
        ...
        return server_1.NextResponse.json({ error: 'Failed to update payment', message: error.message }, { status: 500 });
    }
}, {
