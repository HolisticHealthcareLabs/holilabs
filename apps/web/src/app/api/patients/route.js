...
exports.GET = (0, middleware_1.createProtectedRoute)(async (request, context) => {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Failed to fetch patients' }, { status: 500 });
    }
}, {
...
async function POST(request) {
    try {
        ...
        let validatedData;
        try {
            validatedData = schemas_1.CreatePatientSchema.parse(body);
        }
        catch (error) {
        ...
        const dataHash = (0, hashing_1.generatePatientDataHash)({
            id: tokenId,
        ...
        if (error.code === 'P2002') {
            return server_1.NextResponse.json({ error: 'Patient with this MRN already exists' }, { status: 409 });
        }
        return server_1.NextResponse.json({ error: 'Failed to create patient', details: error.message }, { status: 500 });
    }
}
