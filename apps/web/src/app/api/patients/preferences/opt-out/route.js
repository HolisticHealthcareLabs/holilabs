...
function decryptToken(token) {
    try {
        const decipher = crypto_1.default.createDecipheriv('aes-256-cbc', crypto_1.default.createHash('sha256').update(SECRET_KEY).digest(), Buffer.alloc(16, 0) // IV should be stored with token in production
        );
...
async function GET(request) {
    ...
    await prisma_1.prisma.patientPreferences.upsert({
        where: { patientId },
        update: updateData,
        create: {
            patientId,
