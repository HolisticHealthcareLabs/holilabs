...
async function POST(request) {
    try {
        ...
        if (!email || !emailRegex.test(email)) {
            return server_1.NextResponse.json({ error: 'Please provide a valid email address' }, { status: 400 });
        }
        // Check if Resend is configured
        if (!resend) {
            console.error('Resend API key not configured');
            return server_1.NextResponse.json({ error: 'Email service not configured. Please contact support.' }, { status: 503 });
        }
      ...
      `,
        ...
        return server_1.NextResponse.json({ error: 'Failed to join waitlist. Please try again.' }, { status: 500 });
    }
}
