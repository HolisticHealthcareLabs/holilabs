...
const TranscribeRequestSchema = zod_1.z.object({
    audioUrl: zod_1.z.string().url(),
    generateSOAP: zod_1.z.boolean().default(true),
});
async function POST(request, { params }) {
    try {
        ...
        if (recording.appointment.clinician.id !== session.user.id) {
            return server_1.NextResponse.json({ success: false, error: 'No tienes permiso para transcribir esta grabación' }, { status: 403 });
        }
