...
function getSessionSecret(): Buffer {
  ...
  if (!secret) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      logger.warn('Using development fallback secret for CSRF - DO NOT USE IN PRODUCTION');
      return Buffer.from('dev-fallback-csrf-secret-insecure-do-not-use-in-prod', 'utf8');
    }

    throw new Error(
      'SESSION_SECRET or NEXTAUTH_SECRET must be set in environment. Generate with: openssl rand -hex 32'
    );
  }
...
// ============================================================================
// CSRF TOKEN GENERATION (WITH HMAC SIGNING)
// ============================================================================
 ...
 */
function parseCsrfToken(tokenString: string): CsrfTokenData | null {
