...
async function archiveOldAuditLogs(now: Date): Promise<number> {
  ...
  try {
    const result = await (prisma as any).auditLog.updateMany({
      where: {
...
async function markInactivePatients(now: Date): Promise<number> {
  ...
  try {
    ...
    const inactivePatients = await (prisma as any).patient.findMany({
      where: {
        isActive: true,
        appointments: {
          every: {
    ...
    const result = await (prisma.patient as any).updateMany({
      where: {
        id: { in: inactivePatients.map((p: any) => p.id) },
      },
      data: {
        isActive: false,
        inactivatedAt: now,
