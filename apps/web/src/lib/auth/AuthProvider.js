...
function AuthProvider({ children }) {
    ...
    const [lastActivity, setLastActivity] = (0, react_1.useState)(null);
    const [showTimeoutWarning, setShowTimeoutWarning] = (0, react_1.useState)(false);
    const [timeUntilWarning, setTimeUntilWarning] = (0, react_1.useState)(null);
    const supabase = (0, client_1.createClient)();
    const activityTimerRef = (0, react_1.useRef)(null);
    const warningTimerRef = (0, react_1.useRef)(null);
    const checkIntervalRef = (0, react_1.useRef)(null);
    // Handle timeout logout
    const handleTimeoutLogout = (0, react_1.useCallback)(async () => {
 ...
 */
function SessionTimeoutWarning({ onExtend }) {
    const [countdown, setCountdown] = (0, react_1.useState)(300); // 5 minutes in seconds
    (0, react_1.useEffect)(() => {
    ...
    return (<div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md mx-4 border-2 border-yellow-500">
        ...
        <div className="space-y-3">
          <button onClick={onExtend} className="w-full px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all font-medium shadow-lg hover:shadow-xl">
            ✓ Continuar Sesión
          </button>
