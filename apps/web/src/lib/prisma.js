...
function buildDatabaseUrl(baseUrl) {
    try {
        ...
        logger_1.logger.warn({ event: 'database_url_parse_error', err: error }, 'Could not parse DATABASE_URL, using as-is');
        return baseUrl;
    }
}
/**
...
function createPrismaClient() {
    ...
    if (!process.env.DATABASE_URL) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            logger_1.logger.warn({ event: 'database_url_missing' }, 'DATABASE_URL not set - Prisma client will not be initialized');
        }
        return null;
    }
                ...
                if (data) {
                    for (const field of phiFields) {
                        if (data[field] !== undefined && data[field] !== null) {
                            try {
                                data[field] = (0, encryption_1.encryptPHI)(data[field]);
                            }
                            catch (error) {
                                logger_1.logger.error({ event: 'phi_encryption_failed', field, err: error }, 'Failed to encrypt PHI field');
                                throw error;
                            }
                        }
                    }
                    ...
                    if (params.action === 'upsert' && params.args.update) {
                        for (const field of phiFields) {
                            if (params.args.update[field] !== undefined && params.args.update[field] !== null) {
                                try {
                                    params.args.update[field] = (0, encryption_1.encryptPHI)(params.args.update[field]);
                                }
                                catch (error) {
                                    logger_1.logger.error({ event: 'phi_encryption_failed', field, err: error }, 'Failed to encrypt PHI field');
                                    throw error;
        ...
        if (params.model === 'Patient' && result) {
            const decryptPatient = (patient) => {
                ...
                for (const field of phiFields) {
                    if (patient[field]) {
                        try {
                            patient[field] = (0, encryption_1.decryptPHI)(patient[field]);
                        }
                        catch (error) {
                            logger_1.logger.error({ event: 'phi_decryption_failed', field, err: error }, 'Failed to decrypt PHI field');
 ...
 */
async function connectWithRetry(client, maxRetries = 3, delayMs = 1000) {
...
if (process.env.NODE_ENV === 'production' && prismaClient) {
    process.on('SIGTERM', async () => {
        logger_1.logger.info({ event: 'shutdown', signal: 'SIGTERM' }, 'SIGTERM received, closing database connections...');
        await prismaClient.$disconnect();
        process.exit(0);
    });
    process.on('SIGINT', async () => {
        logger_1.logger.info({ event: 'shutdown', signal: 'SIGINT' }, 'SIGINT received, closing database connections...');
        await prismaClient.$disconnect();
        process.exit(0);
    });
}
