...
exports.routeAIRequest = routeAIRequest;
...
function analyzeComplexity(messages) {
    ...
    const hasCriticalKeywords = criticalKeywords.some(keyword => lastMessage.toLowerCase().includes(keyword));
    if (hasCriticalKeywords) {
        return 'critical';
    }
    // Check for complex cases
    const hasComplexKeywords = complexKeywords.some(keyword => lastMessage.toLowerCase().includes(keyword));
 ...
 */
function getEstimatedCost(provider, tokenCount = 2000) {
 ...
 */
async function routeAIRequest(request, config = {}) {
    ...
    let response = await (0, chat_1.chat)({
        ...request,
    ...
    console.log(`[AI Router] Request completed | Provider: ${selectedProvider} | ` +
...
exports.AIRouter = {
     ...
     */
    general: (messages) => routeAIRequest({ messages, provider: 'gemini' }),
    /**
     * For differential diagnosis (uses Claude for accuracy)
     */
    differential: (messages) => routeAIRequest({
        messages,
        provider: 'claude',
    }),
    /**
     ...
     */
    drugInteractions: (messages) => routeAIRequest({
        messages,
        provider: 'claude',
    }),
    /**
     ...
     */
    treatment: (messages) => routeAIRequest({ messages, provider: 'gemini' }),
    /**
     * Auto-route based on complexity analysis
     */
    auto: (messages) => routeAIRequest({ messages }),
};
