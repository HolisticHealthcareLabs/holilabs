...
import { checkDrugInteractions } from '../rules/drug-interactions';
...
export class CDSEngine {
  ...
  /**
   * Evaluate CDS rules for a given context
   */
      ...
      try {
        ...
        if (rule.condition(context)) {
          ...
          if (alert) {
            alerts.push(alert);
            rulesFired++;
            console.log(`✅ [CDS Engine] Rule fired: ${rule.name} (${alert.severity})`);
          }
        }
      } catch (error) {
