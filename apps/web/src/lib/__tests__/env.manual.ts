...
try {
  const env = validateEnv({ exitOnError: false, skipDatabaseCheck: true });
...
try {
  console.log(`   Blockchain enabled: ${isFeatureEnabled('ENABLE_BLOCKCHAIN')}`);
  console.log(`   Sentry enabled: ${isFeatureEnabled('NEXT_PUBLIC_SENTRY_DSN')}`);
  console.log(`   Redis enabled: ${isFeatureEnabled('REDIS_URL') || isFeatureEnabled('UPSTASH_REDIS_REST_URL')}`);
...
try {
  ...
  (validateEnv as any).cachedEnv = null;
