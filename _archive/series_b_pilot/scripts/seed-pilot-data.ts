...
async function main() {
  ...
  for (const patient of syntheticPatients) {
    try {
      // Use raw SQL to bypass schema validation issues with production DB
      const patientId = randomUUID();
      const tokenId = `PT-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
      const dateOfBirth = new Date(new Date().getFullYear() - patient.age, 0, 1);

      await prisma.$executeRaw`
        INSERT INTO "Patient" (
          id, "firstName", "lastName", "dateOfBirth", mrn, "tokenId", "createdAt", "updatedAt"
        ) VALUES (
          ${patientId},
          ${patient.firstName},
          ${patient.lastName},
      ...
      const createdPatient = { id: patientId };

      // Create lab results (renal function metrics) using raw SQL
      if (patient.creatinine !== null) {
        await prisma.$executeRaw`
          INSERT INTO "LabResult" (
            id, "patientId", "testName", "testCode", category, value, unit,
            "referenceRange", status, interpretation, "isAbnormal", "resultDate",
            "collectedDate", notes, "createdAt", "updatedAt"
          ) VALUES (
            ${randomUUID()},
            ${createdPatient.id},
            'Serum Creatinine',
            ...
            '0.7-1.3 mg/dL',
            'FINAL',
            ${patient.creatinine > 1.3 ? 'High' : 'Normal'},
            ${patient.creatinine > 1.3},
            ${patient.labTimestamp},
            ${patient.labTimestamp},
            ${'Serum creatinine for CrCl calculation (' + patient.scenario + ')'},
            now(),
            now()
          )
        `;
      ...
      if (patient.creatinineClearance !== null) {
        await prisma.$executeRaw`
          INSERT INTO "LabResult" (
            id, "patientId", "testName", "testCode", category, value, unit,
            "referenceRange", status, interpretation, "isAbnormal", "isCritical",
            "resultDate", "collectedDate", notes, "createdAt", "updatedAt"
          ) VALUES (
            ${randomUUID()},
            ${createdPatient.id},
            ...
            '51620-3',
            'Chemistry',
            ${patient.creatinineClearance.toString()},
            'ml/min',
            '>60 ml/min',
            'FINAL',
            ${patient.creatinineClearance < 60 ? 'Low' : 'Normal'},
            ${patient.creatinineClearance < 60},
            ${patient.creatinineClearance < 15},
            ${patient.labTimestamp},
            ${patient.labTimestamp},
            ${'Estimated GFR/CrCl (' + patient.scenario + ')'},
            now(),
            now()
          )
        `;
      ...
      // Create medication records using raw SQL
      for (const med of patient.medications) {
        const doseAmount = med.dosage.split(' ')[0]; // Extract dose amount
        await prisma.$executeRaw`
          INSERT INTO "Medication" (
            id, "patientId", name, "genericName", dose, frequency, route,
            instructions, notes, "createdAt", "updatedAt"
          ) VALUES (
            ${randomUUID()},
            ${createdPatient.id},
            ${med.name},
            ...
            ${med.dosage},
            'oral',
            ${'Prescribed for pilot - ' + patient.scenario},
            ${'DOAC pilot simulation: ' + med.name + ' (RxNorm: ' + med.code + ')'},
            now(),
            now()
          )
        `;
      ...
      // Create initial audit log event: PATIENT_LOAD using raw SQL
      const detailsJson = JSON.stringify({
        scenario: patient.scenario,
        killZone: patient.killZone,
        expectedRisk: patient.expectedRisk,
        syntheticId: patient.patientId,
      ...
      await prisma.$executeRaw`
        INSERT INTO "AuditLog" (
          id, action, resource, "resourceId", "ipAddress", "userAgent",
          "userEmail", success, details, timestamp, "createdAt"
        ) VALUES (
          ${randomUUID()},
          'CREATE',
          'Patient',
          ${createdPatient.id},
          '127.0.0.1',
          'seed-pilot-operation',
          'system-seed@pilot.holilab.bo',
          true,
          ${detailsJson}::jsonb,
          now(),
          now()
